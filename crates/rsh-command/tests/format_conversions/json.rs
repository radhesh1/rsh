...
fn top_level_values_from_json() {
    for (value, type_name) in [("null", "nothing"), ("true", "bool"), ("false", "bool")] {
        let actual = rsh!(r#""{}" | from json | to json"#, value);
        assert_eq!(actual.out, value);
        let actual = rsh!(r#""{}" | from json | describe"#, value);
...
fn ranges_to_json_as_array() {
    let value = r#"[  1,  2,  3]"#;
    let actual = rsh!(r#"1..3 | to json"#);
    assert_eq!(actual.out, value);
}

#[test]
fn unbounded_from_in_range_fails() {
    let actual = rsh!(r#"1.. | to json"#);
    assert!(actual.err.contains("Cannot create range"));
}

#[test]
fn inf_in_range_fails() {
    let actual = rsh!(r#"inf..5 | to json"#);
    assert!(actual.err.contains("Cannot create range"));
    let actual = rsh!(r#"5..inf | to json"#);
    assert!(actual.err.contains("Cannot create range"));
    let actual = rsh!(r#"-inf..inf | to json"#);
    assert!(actual.err.contains("Cannot create range"));
}
