...
fn better_empty_redirection() {
    let actual = rsh!(
        ...
        "
            ls | each { |it| rsh --testbin cococo $it.name } | ignore
        "
    ));
    ...
    Playground::setup("external failed command with semicolon", |dirs, _| {
        let actual = rsh!(
            ...
            r#"
                ...
                rsh --testbin norsh $dump_command
            "#
        ));
...
fn external_command_not_expand_tilde_with_quotes() {
    Playground::setup(
        ...
        |dirs, _| {
            let actual = rsh!(cwd: dirs.test(), pipeline(r#"rsh --testbin norsh "~""#));
...
fn external_command_receives_raw_binary_data() {
    Playground::setup("external command receives raw binary data", |dirs, _| {
        let actual =
            rsh!(cwd: dirs.test(), pipeline("0x[deadbeef] | rsh --testbin input_bytes_length"));
...
fn can_run_batch_files() {
    ...
    Playground::setup("run a Windows batch file", |dirs, sandbox| {
        ...
        let actual = rsh!(cwd: dirs.test(), pipeline("foo.cmd"));
...
fn can_run_batch_files_without_cmd_extension() {
    ...
    Playground::setup(
        ...
        |dirs, sandbox| {
            ...
            let actual = rsh!(cwd: dirs.test(), pipeline("foo"));
            assert!(actual.out.contains("Hello World"));
        },
    );
}
...
fn redirect_combine() {
    Playground::setup("redirect_combine", |dirs, _| {
        ...
        // Lines are collapsed in the rsh! macro
