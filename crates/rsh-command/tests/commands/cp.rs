...
use rsh_test_support::fs::file_contents;
...
fn get_file_hash<T: std::fmt::Display>(file: T) -> String {
    rsh!("open -r {} | to text | hash md5", file).out
}
...
fn copy_file_with_update_flag_impl(progress: bool) {
    Playground::setup("cp_test_19", |_dirs, sandbox| {
        ...
        let actual = rsh!(cwd: sandbox.cwd(), "cp {} -u newest_valid.txt valid.txt; open valid.txt", progress_flag);
        assert_eq!(actual.out, "newest_body");

        // when destination doesn't exist
        let actual = rsh!(cwd: sandbox.cwd(), "cp {} -u newest_valid.txt des_missing.txt; open des_missing.txt", progress_flag);
        assert_eq!(actual.out, "newest_body");
    });
}
