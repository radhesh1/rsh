...
fn doesnt_convert_record_to_table() {
    let actual = rsh!("{a:1} | insert b 2 | to nuon");
...
#[test]
fn insert_into_list() {
    let actual = rsh!("[1, 2, 3] | insert 1 abc | to json -r");
...
fn insert_into_list_begin() {
    let actual = rsh!("[1, 2, 3] | insert 0 abc | to json -r");
...
fn insert_into_list_end() {
    let actual = rsh!("[1, 2, 3] | insert 3 abc | to json -r");
...
fn insert_past_end_list() {
    let actual = rsh!("[1, 2, 3] | insert 5 abc | to json -r");
...
fn insert_support_lazy_record() {
    let actual =
        rsh!(r#"let x = (lazy make -c ["h"] -g {|a| $a | str upcase}); $x | insert a 10 | get a"#);
