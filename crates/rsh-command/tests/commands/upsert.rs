...
fn doesnt_convert_record_to_table() {
    let actual = rsh!("{a:1} | upsert a 2 | to nuon");
...
fn upsert_uses_enumerate_index_updating() {
    let actual = rsh!("[[a]; [7] [6]] | enumerate | upsert a {|el| $el.index + 1 + $el.item.a } | flatten | to nuon");
...
fn index_does_not_exist() {
    let actual = rsh!("[1,2,3] | upsert 4 4");
...
fn upsert_support_lazy_record() {
    let actual =
        rsh!(r#"let x = (lazy make -c ["h"] -g {|a| $a | str upcase}); $x | upsert h 10 | get h"#);
    assert_eq!(actual.out, "10");

    let actual =
        rsh!(r#"let x = (lazy make -c ["h"] -g {|a| $a | str upcase}); $x | upsert aa 10 | get aa"#);
    assert_eq!(actual.out, "10");
}

#[test]
fn deep_cell_path_creates_all_nested_records() {
    let actual = rsh!(r#"{a: {}} | insert a.b.c 0 | get a.b.c"#);
