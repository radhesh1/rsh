...
fn sort_primitive_values() {
    ...
    assert_eq!(actual.out, "authors = [\"The rsh Project Developers\"]");
}

#[test]
fn sort_different_types() {
    let actual = rsh!("[a, 1, b, 2, c, 3, [4, 5, 6], d, 4, [1, 2, 3]] | sort | to json --raw");
...
#[test]
fn sort_natural() {
    let actual = rsh!("['1' '2' '3' '4' '5' '10' '100'] | sort -n | to nuon");
...
fn sort_record_natural() {
    let actual = rsh!("{10:0,99:0,1:0,9:0,100:0} | sort -n | to nuon");
...
fn sort_record_insensitive() {
    let actual = rsh!("{abe:1,zed:2,ABE:3} | sort -i | to nuon");
...
fn sort_record_insensitive_reverse() {
    let actual = rsh!("{abe:1,zed:2,ABE:3} | sort -ir | to nuon");
...
fn sort_record_values_natural() {
    let actual = rsh!(r#"{1:"1",2:"2",4:"100",3:"10"} | sort -vn | to nuon"#);
...
fn sort_record_values_insensitive() {
    let actual = rsh!("{1:abe,2:zed,3:ABE} | sort -vi | to nuon");
...
fn sort_record_values_insensitive_reverse() {
    let actual = rsh!("{1:abe,2:zed,3:ABE} | sort -vir | to nuon");
