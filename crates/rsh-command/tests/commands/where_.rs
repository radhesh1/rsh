...
fn where_inside_block_works() {
    let actual = rsh!("{|x| ls | where $it =~ 'foo' } | describe");
...
fn filters_with_0_arity_block() {
    let actual = rsh!("[1 2 3 4] | where {|| $in < 3 } | to nuon");
...
fn filters_with_1_arity_block() {
    let actual = rsh!("[1 2 3 6 7 8] | where {|e| $e < 5 } | to nuon");
...
fn where_uses_enumerate_index() {
    let actual = rsh!("[7 8 9 10] | enumerate | where {|el| $el.index < 2 } | to nuon");
...
fn fail_on_non_iterator() {
    let actual = rsh!(r#"{"name": "foo", "size": 3} | where name == "foo""#);
...
#[test]
fn where_gt_null() {
    let actual = rsh!("[{foo: 123} {}] | where foo? > 10 | to nuon");
