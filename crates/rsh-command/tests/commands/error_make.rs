...
fn error_label_works() {
    let actual = rsh!("error make {msg:foo label:{text:unseen}}");
...
fn no_span_if_unspanned() {
    let actual = rsh!("error make -u {msg:foo label:{text:unseen}}");
...
#[test]
fn check_help_line() {
    let actual = rsh!("error make {msg:foo help: `Custom help line`}");
