    ...
    Playground::setup("select_test_2", |dirs, sandbox| {
        ...
        let actual = rsh!(
            ...
            r#"
                ...
                | select rsh."0xATYKARNU" rsh.committers.name rsh.releases.version
                | get rsh_releases_version
                | where $it > "0.8"
                | get 0
            "#
        ));
...
fn fails_if_given_unknown_column_name() {
    ...
    assert!(actual.err.contains("rsh::shell::name_not_found"));
}
...
fn select_ignores_errors_successfully1() {
    let actual = rsh!("[{a: 1, b: 2} {a: 3, b: 5} {a: 3}] | select b? | length");
...
fn select_ignores_errors_successfully2() {
    let actual = rsh!("[{a: 1} {a: 2} {a: 3}] | select b? | to nuon");
...
fn select_ignores_errors_successfully3() {
    let actual = rsh!("sys | select invalid_key? | to nuon");
...
#[test]
fn select_failed1() {
    let actual = rsh!("[{a: 1, b: 2} {a: 3, b: 5} {a: 3}] | select b ");
...
#[test]
fn select_failed2() {
    let actual = rsh!("[{a: 1} {a: 2} {a: 3}] | select b");
...
fn select_repeated_rows() {
    let actual = rsh!("[[a b c]; [1 2 3] [4 5 6] [7 8 9]] | select 0 0 | to nuon");
...
fn select_repeated_column() {
    let actual = rsh!("[[a b c]; [1 2 3] [4 5 6] [7 8 9]] | select a a | to nuon");
...
fn select_on_empty_list_returns_empty_list() {
    // once with a List
    let actual = rsh!("[] | select foo | to nuon");
    assert_eq!(actual.out, "[]");

    // and again with a ListStream
    let actual = rsh!("[] | each {|i| $i} | select foo | to nuon");
    assert_eq!(actual.out, "[]");
}
...
fn select_single_row_with_variable() {
    let actual = rsh!("let idx = 2;[{a: 1, b: 2} {a: 3, b: 5} {a: 3}] | select $idx | to nuon");
