...
fn lists_with_directory_flag() {
    Playground::setup("ls_test_flag_directory_1", |dirs, sandbox| {
        sandbox
            .within("dir_files")
            .with_files(vec![EmptyFile("rsh.json")])
            .within("dir_empty");
        let actual = rsh!(
            cwd: dirs.test(), pipeline(
            r#"
        ...
        let expected = [".", ".", "..", "../dir_files", "../dir_files/rsh.json"].join("");
        #[cfg(windows)]
...
/// This test confirms that rsh can work around this successfully.
#[test]
#[cfg(windows)]
...
fn list_a_directory_not_exists() {
    Playground::setup("ls_test_directory_not_exists", |dirs, _sandbox| {
        let actual = rsh!(cwd: dirs.test(), "ls a_directory_not_exists");
