...
fn simple_get_record() {
    let actual = rsh!(r#"({foo: 'bar'} | get foo) == "bar""#);
...
fn fetches_a_row() {
    Playground::setup("get_test_1", |dirs, sandbox| {
        sandbox.with_files(vec![FileWithContent(
            "sample.toml",
            r#"
                rsh_party_venue = "zion"
            "#,
        )]);

        let actual = rsh!( cwd: dirs.test(), "open sample.toml | get rsh_party_venue");
...
fn fetches_by_index() {
    Playground::setup("get_test_2", |dirs, sandbox| {
        ...
        let actual = rsh!( cwd: dirs.test(), "open sample.toml | get package.authors.2");
...
fn fetches_by_column_path() {
    Playground::setup("get_test_3", |dirs, sandbox| {
        ...
        let actual = rsh!( cwd: dirs.test(), "open sample.toml | get package.name");
...
fn column_paths_are_either_double_quoted_or_regular_unquoted_words_separated_by_dot() {
    Playground::setup("get_test_4", |dirs, sandbox| {
        ...
        let actual = rsh!( cwd: dirs.test(), r#"open sample.toml | get package."9999" | length"#);
...
fn errors_fetching_by_column_not_present() {
    Playground::setup("get_test_6", |dirs, sandbox| {
        ...
        let actual = rsh!( cwd: dirs.test(), "open sample.toml | get taco");
...
fn errors_fetching_by_column_using_a_number() {
    Playground::setup("get_test_7", |dirs, sandbox| {
        ...
        let actual = rsh!( cwd: dirs.test(), "open sample.toml | get spanish_lesson.0");
...
fn errors_fetching_by_index_out_of_bounds() {
    Playground::setup("get_test_8", |dirs, sandbox| {
        sandbox.with_files(vec![FileWithContent(
            ...
            r#"
                [spanish_lesson]
                sentence_words = ["Yo", "quiero", "tacorsh"]
            "#,
        )]);
...
fn get_does_not_delve_too_deep_in_nested_lists() {
    let actual = rsh!("[[{foo: bar}]] | get foo");
...
fn ignore_errors_works() {
    let actual = rsh!(r#" let path = "foo"; {} | get -i $path | to nuon "#);
