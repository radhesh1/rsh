...
fn with_env_extends_environment() {
    let actual = rsh!("with-env [FOO BARRRR] {echo $env} | get FOO");
...
fn with_env_shorthand() {
    let actual = rsh!("FOO=BARRRR echo $env | get FOO");
...
fn shorthand_doesnt_reorder_arguments() {
    let actual = rsh!("FOO=BARRRR rsh --testbin cococo first second");
...
fn with_env_and_shorthand_same_result() {
    let actual_shorthand = rsh!("FOO='BARRRR' echo $env | get FOO");

    let actual_normal = rsh!("with-env [FOO BARRRR] {echo $env} | get FOO");
...
fn test_redirection2() {
    let actual =
        rsh!("let x = (FOO=BAR rsh --testbin cococo niceenvvar); $x | str trim | str length");
