...
fn capture_errors_works_for_external() {
    let actual = rsh!("do -c {rsh --testbin fail}");
...
fn capture_errors_works_for_external_with_pipeline() {
    let actual = rsh!("do -c {rsh --testbin fail} | echo `text`");
    assert!(actual.err.contains("External command failed"));
    assert_eq!(actual.out, "");
}
...
fn capture_errors_works_for_external_with_semicolon() {
    let actual = rsh!(r#"do -c {rsh --testbin fail}; echo `text`"#);
    assert!(actual.err.contains("External command failed"));
    assert_eq!(actual.out, "");
}
...
fn do_with_semicolon_break_on_failed_external() {
    let actual = rsh!(r#"do { rsh --not_exist_flag }; `text`"#);
...
fn ignore_shell_errors_works_for_external_with_semicolon() {
    let actual = rsh!(r#"do -s { open asdfasdf.txt }; "text""#);
...
fn ignore_program_errors_works_for_external_with_semicolon() {
    let actual = rsh!(r#"do -p { rsh -c 'exit 1' }; "text""#);
...
fn ignore_error_should_work_for_external_command() {
    let actual = rsh!(r#"do -i { rsh --testbin fail asdf }; echo post"#);
...
#[test]
#[cfg(not(windows))]
fn capture_error_with_too_much_stderr_not_hang_rsh() {
...
#[test]
#[cfg(not(windows))]
fn capture_error_with_too_much_stdout_not_hang_rsh() {
    use rsh_test_support::fs::Stub::FileWithContent;
    use rsh_test_support::pipeline;
    use rsh_test_support::playground::Playground;
...
#[test]
#[cfg(not(windows))]
fn capture_error_with_both_stdout_stderr_messages_not_hang_rsh() {
    use rsh_test_support::fs::Stub::FileWithContent;
    use rsh_test_support::playground::Playground;
    Playground::setup(
...
fn ignore_error_works_with_list_stream() {
    let actual = rsh!(r#"do -i { ["a", null, "b"] | ansi strip }"#);
