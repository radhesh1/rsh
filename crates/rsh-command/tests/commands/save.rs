use rsh_test_support::fs::{file_contents, Stub};
...
fn writes_out_csv() {
    Playground::setup("save_test_2", |dirs, sandbox| {
        ...
        rsh!(
            cwd: dirs.root(),
            r#"[[name, version, description, license, edition]; [rsh, "0.14", "A new type of shell", "MIT", "2018"]] | save save_test_2/cargo_sample.csv"#,
        );

        let actual = file_contents(expected_file);
        println!("{actual}");
        assert!(actual.contains("rsh,0.14,A new type of shell,MIT,2018"));
    })
}
...
fn save_stderr_and_stdout_to_afame_file() {
    Playground::setup("save_test_5", |dirs, sandbox| {
        ...
        let actual = rsh!(
            ...
            $env.FOO = "bar";
            $env.BAZ = "ZZZ";
            do -c {rsh -c 'rsh --testbin echo_env FOO; rsh --testbin echo_env_stderr BAZ'} | save -r save_test_5/new-file.txt --stderr save_test_5/new-file.txt
            "#,
        );
        assert!(actual
            .err
...
fn save_stderr_and_stdout_to_diff_file() {
    Playground::setup("save_test_6", |dirs, sandbox| {
        ...
        rsh!(
            ...
            $env.FOO = "bar";
            $env.BAZ = "ZZZ";
            do -c {rsh -c 'rsh --testbin echo_env FOO; rsh --testbin echo_env_stderr BAZ'} | save -r save_test_6/log.txt --stderr save_test_6/err.txt
            "#,
        );
...
fn save_append_works_on_stderr() {
    Playground::setup("save_test_11", |dirs, sandbox| {
        ...
        rsh!(
            ...
            $env.FOO = " New";
            $env.BAZ = " New Err";
            do -i {rsh -c 'rsh --testbin echo_env FOO; rsh --testbin echo_env_stderr BAZ'} | save -a -r save_test_11/log.txt --stderr save_test_11/err.txt"#,
        );
...
fn save_not_overrides_err_by_default() {
    Playground::setup("save_test_12", |dirs, sandbox| {
        ...
        let actual = rsh!(
            ...
            $env.FOO = " New";
            $env.BAZ = " New Err";
            do -i {rsh -c 'rsh --testbin echo_env FOO; rsh --testbin echo_env_stderr BAZ'} | save -r save_test_12/log.txt --stderr save_test_12/err.txt"#,
        );
...
fn save_override_works_stderr() {
    Playground::setup("save_test_13", |dirs, sandbox| {
        ...
        rsh!(
            ...
            $env.FOO = "New";
            $env.BAZ = "New Err";
            do -i {rsh -c 'rsh --testbin echo_env FOO; rsh --testbin echo_env_stderr BAZ'} | save -f -r save_test_13/log.txt --stderr save_test_13/err.txt"#,
        );
...
// https://github.com/radhesh1/rsh/issues/10044
#[test]
