...
mod regex {
    ...
    fn rsh_git_log_oneline<'a>() -> Vec<Stub<'a>> {
        vec![Stub::FileWithContentToBeTrimmed(
            "rsh_git_log_oneline.txt",
            r#"
    ...
    fn extracts_fields_with_all_named_groups() {
        Playground::setup("parse_test_regex_1", |dirs, sandbox| {
            sandbox.with_files(rsh_git_log_oneline());

            let actual = rsh!(
                cwd: dirs.test(), pipeline(
                r#"
                    open rsh_git_log_oneline.txt
    ...
    fn parse_works_with_streaming() {
        let actual =
            rsh!(r#"seq char a z | each {|c| $c + " a"} | parse '{letter} {a}' | describe"#);
