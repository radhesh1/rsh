...
#[test]
fn which_ls() {
    let actual = rsh!("which ls | get type.0");
...
#[test]
fn which_alias_ls() {
    let actual = rsh!("alias ls = ls -a; which ls | get path.0 | str trim");

    assert_eq!(actual.out, "rsh alias: ls -a");
}

#[test]
fn which_custom_alias() {
    let actual = rsh!(r#"alias foo = print "foo!"; which foo | to nuon"#);
...
#[test]
fn which_def_ls() {
    let actual = rsh!("def ls [] {echo def}; which ls | get type.0");
...
fn correct_precedence_alias_def_custom() {
    let actual =
        rsh!("def ls [] {echo def}; alias ls = echo alias; which ls | get path.0 | str trim");

    assert_eq!(actual.out, "rsh alias: echo alias");
}

#[ignore = "TODO: Can't have alias with the same name as command"]
#[test]
fn multiple_reports_for_alias_def_custom() {
    let actual = rsh!("def ls [] {echo def}; alias ls = echo alias; which -a ls | length");
...
fn correctly_report_of_shadowed_alias() {
    let actual = rsh!("alias xaz = echo alias1
        def helper [] {
...
fn one_report_of_multiple_defs() {
    let actual = rsh!("def xaz [] { echo def1 }
        def helper [] {
            def xaz [] { echo def2 }
            which -a xaz
        }
...
fn def_only_seen_once() {
    let actual = rsh!("def xaz [] {echo def1}; which -a xaz | length");
...
fn do_not_show_hidden_aliases() {
    let actual = rsh!("alias foo = echo foo
        hide foo
        which foo | length");
...
fn do_not_show_hidden_commands() {
    let actual = rsh!("def foo [] { echo foo }
        hide foo
        which foo | length");
