...
fn http_get_with_custom_headers_as_records() {
    ...
    let _json_response = rsh!(format!(
        "http get -H {{content-type: application/json}} {url}",
        url = server.url()
    ));

    let _text_response = rsh!(format!(
...
fn http_get_expired_cert_fails() {
    let actual = rsh!("http get https://expired.badssl.com/");
    assert!(actual.err.contains("network_failure"));
}

#[test]
fn http_get_expired_cert_override() {
    let actual = rsh!("http get --insecure https://expired.badssl.com/");
    assert!(actual.out.contains("<html>"));
}

#[test]
fn http_get_self_signed_fails() {
    let actual = rsh!("http get https://self-signed.badssl.com/");
    assert!(actual.err.contains("network_failure"));
}

#[test]
fn http_get_self_signed_override() {
    let actual = rsh!("http get --insecure https://self-signed.badssl.com/");
    assert!(actual.out.contains("<html>"));
}
