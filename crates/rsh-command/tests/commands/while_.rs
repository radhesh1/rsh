...
fn while_doesnt_auto_print_in_each_iteration() {
    let actual = rsh!("mut total = 0; while $total < 2 { $total = $total + 1; 1 }");
...
fn while_break_on_external_failed() {
    let actual =
        rsh!("mut total = 0; while $total < 2 { $total = $total + 1; print 1; rsh --testbin fail }");
...
fn failed_while_should_break_running() {
    let actual =
        rsh!("mut total = 0; while $total < 2 { $total = $total + 1; rsh --testbin fail }; print 3");
