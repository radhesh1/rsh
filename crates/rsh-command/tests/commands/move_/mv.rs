use rsh_test_support::fs::{files_exist_at, Stub::EmptyFile, Stub::FileWithContent};
...
// We were bitten badly by this once: https://github.com/radhesh1/rsh/issues/6583
...
fn mv_with_update_flag() {
    Playground::setup("mv_with_update_flag", |_dirs, sandbox| {
        ...
        let actual = rsh!(cwd: sandbox.cwd(), "mv -uf newest_valid.txt valid.txt; open valid.txt");
        assert_eq!(actual.out, "newest_body");

        // when destination doesn't exist
        sandbox.with_files(vec![FileWithContent("newest_valid.txt", "newest_body")]);
        let actual = rsh!(cwd: sandbox.cwd(), "mv -uf newest_valid.txt des_missing.txt; open des_missing.txt");
        assert_eq!(actual.out, "newest_body");
    });
}
