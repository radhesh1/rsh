...
fn checks_all_rows_are_false_with_param() {
    let actual = rsh!(" [1, 2, 3, 4] | all { |a| $a >= 5 } ");
...
fn works_with_1_param_blocks() {
    let actual = rsh!("[1 2 3] | all {|e| print $e | true }");
...
fn works_with_0_param_blocks() {
    let actual = rsh!("[1 2 3] | all {|| print $in | true }");
...
fn early_exits_with_1_param_blocks() {
    let actual = rsh!("[1 2 3] | all {|e| print $e | false }");
...
fn early_exits_with_0_param_blocks() {
    let actual = rsh!("[1 2 3] | all {|| print $in | false }");

    assert_eq!(actual.out, "1false");
}

#[test]
fn all_uses_ershmerate_index() {
    let actual = rsh!("[7 8 9] | enumerate | all {|el| print $el.index | true }");
...
fn unique_env_each_iteration() {
    let actual = rsh!(
        ...
        "[1 2] | all {|| print ($env.PWD | str ends-with 'formats') | cd '/' | true } | to rshon"
    );
