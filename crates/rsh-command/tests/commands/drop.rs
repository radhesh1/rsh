...
fn drop_columns_positive_value() {
    let actual = rsh!("echo [[a, b];[1,2]] | drop column -1");
...
fn more_rows_than_table_has() {
    let actual = rsh!("[date] | drop 50 | length");
...
fn nth_range_inclusive() {
    let actual = rsh!("echo 10..15 | drop nth (2..3) | to json --raw");
...
fn nth_range_exclusive() {
    let actual = rsh!("echo 10..15 | drop nth (1..<3) | to json --raw");
...
fn nth_missing_first_argument() {
    let actual = rsh!("echo 10..15 | drop nth \"\"");
...
#[test]
fn record() {
    let actual = rsh!("{a: 1, b: 2} | drop column | to nuon");
...
fn more_columns_than_record_has() {
    let actual = rsh!("{a: 1, b: 2} | drop column 3 | to nuon");
