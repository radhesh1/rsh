...
fn doesnt_convert_record_to_table() {
    let actual = rsh!("{a:1} | update a 2 | to nuon");
...
fn update_support_lazy_record() {
    let actual =
        rsh!(r#"let x = (lazy make -c ["h"] -g {|a| $a | str upcase}); $x | update h 10 | get h"#);
