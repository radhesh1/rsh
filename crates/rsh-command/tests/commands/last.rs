...
fn gets_last_rows_by_amount() {
    Playground::setup("last_test_1", |dirs, sandbox| {
        ...
        let actual = rsh!(cwd: dirs.test(), "ls | last 3 | length");
...
fn gets_last_row_when_no_amount_given() {
    Playground::setup("last_test_2", |dirs, sandbox| {
        ...
        let actual = rsh!(cwd: dirs.test(), "ls | last | values | length");
...
fn requests_more_rows_than_table_has() {
    let actual = rsh!("[date] | last 50 | length");
...
fn gets_last_row_as_list_when_amount_given() {
    let actual = rsh!("[1, 2, 3] | last 1 | describe");
...
#[test]
fn gets_last_bytes() {
    let actual = rsh!("(0x[aa bb cc] | last 2) == 0x[bb cc]");
...
#[test]
fn gets_last_byte() {
    let actual = rsh!("0x[aa bb cc] | last");
