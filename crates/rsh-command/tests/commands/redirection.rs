use rsh_test_support::fs::{file_contents, Stub::FileWithContent};
...
fn redirect_outerr() {
    Playground::setup("redirect_outerr_test", |dirs, _sandbox| {
        ...
        let output = rsh!(cwd: dirs.test(), "cat a");
...
fn redirect_outerr() {
    Playground::setup("redirect_outerr_test", |dirs, _sandbox| {
        ...
        let output = rsh!(cwd: dirs.test(), "(open a | str stats).bytes >= 16");
...
#[test]
fn same_target_redirection_with_too_much_stderr_not_hang_rsh() {
    ...
    Playground::setup("external with many stderr message", |dirs, sandbox| {
        ...
        rsh!(
            cwd: dirs.test(), pipeline(
                ...
                rsh --testbin echo_env_stderr LARGE out+err> another_large_file.txt
                "
            ),
        );
...
fn no_duplicate_redirection() {
    let actual = rsh!("echo 3 o> a.txt o> a.txt");
    assert!(actual.err.contains("Redirection can be set only once"));
    let actual = rsh!("echo 3 e> a.txt e> a.txt");
    assert!(actual.err.contains("Redirection can be set only once"));
}
