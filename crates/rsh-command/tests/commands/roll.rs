...
mod columns {
    ...
    fn can_roll_the_cells_only_keeping_the_header_names() {
        let four_bitstring = bitstring_to_rsh_row_pipeline("00000100");
    ...
    fn shift_three_bits_to_the_left_to_bitstring(bits: &str) -> String {
        // this pipeline takes the bitstring and outputs a rsh row literal
        ...
        //  ]
        //
        let bitstring_as_rsh_row_pipeline = bitstring_to_rsh_row_pipeline(bits);

        // this pipeline takes the rsh bitstring row literal, computes it's
        // decimal value.
        let rsh_row_literal_bitstring_to_decimal_value_pipeline = pipeline(
            r#"
        ...
        println!(
            "{bitstring_as_rsh_row_pipeline} | roll left --by 3 | {rsh_row_literal_bitstring_to_decimal_value_pipeline}"
        );
        rsh!(
            format!("{bitstring_as_rsh_row_pipeline} | roll left --by 3 | {rsh_row_literal_bitstring_to_decimal_value_pipeline}")
        ).out
    }

    fn bitstring_to_rsh_row_pipeline(bits: &str) -> String {
        format!(
            "echo '{}' | {}",
            bits,
            pipeline(
