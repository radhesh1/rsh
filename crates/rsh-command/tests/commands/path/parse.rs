...
#[test]
fn parses_into_correct_rshmber_of_columns() {
    let actual = rsh!(
        cwd: "tests", pipeline(
        r#"
            echo 'home/viking/spam.txt'
...
#[test]
fn const_path_parse() {
    let actual = rsh!("const name = ('spam/eggs.txt' | path parse); $name.parent");
    assert_eq!(actual.out, "spam");

    let actual = rsh!("const name = ('spam/eggs.txt' | path parse); $name.stem");
    assert_eq!(actual.out, "eggs");

    let actual = rsh!("const name = ('spam/eggs.txt' | path parse); $name.extension");
