...
fn works_with_1_param_blocks() {
    let actual = rsh!("[1 2 3] | any {|e| print $e | false }");
...
fn works_with_0_param_blocks() {
    let actual = rsh!("[1 2 3] | any {|| print $in | false }");
...
fn early_exits_with_1_param_blocks() {
    let actual = rsh!("[1 2 3] | any {|e| print $e | true }");
...
fn early_exits_with_0_param_blocks() {
    let actual = rsh!("[1 2 3] | any {|| print $in | true }");
...
fn any_uses_enumerate_index() {
    let actual = rsh!("[7 8 9] | enumerate | any {|el| print $el.index | false }");
