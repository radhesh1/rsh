...
fn into_int_filesize() {
    let actual = rsh!("echo 1kb | into int | each { |it| $it / 1000 }");
...
fn into_int_filesize2() {
    let actual = rsh!("echo 1kib | into int | each { |it| $it / 1024 }");
...
#[test]
fn into_int_int() {
    let actual = rsh!("echo 1024 | into int | each { |it| $it / 1024 }");
...
#[test]
fn into_int_binary() {
    let actual = rsh!("echo 0x[01010101] | into int");
