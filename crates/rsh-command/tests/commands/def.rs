...
fn def_errors_with_no_space_between_params_and_name_1() {
    let actual = rsh!("def test-command[] {}");
...
fn def_errors_with_no_space_between_params_and_name_2() {
    let actual = rsh!("def --env test-command() {}");
...
fn def_errors_with_multiple_short_flags() {
    let actual = rsh!("def test-command [ --long(-l)(-o) ] {}");
...
fn def_errors_with_comma_before_alternative_short_flag() {
    let actual = rsh!("def test-command [ --long, (-l) ] {}");
...
fn def_errors_with_comma_before_equals() {
    let actual = rsh!("def test-command [ foo, = 1 ] {}");
...
fn def_errors_with_comma_before_colon() {
    let actual = rsh!("def test-command [ foo, : int ] {}");
...
fn def_errors_with_multiple_types() {
    let actual = rsh!("def test-command [ foo:int:string ] {}");
...
fn def_errors_with_multiple_commas() {
    let actual = rsh!("def test-command [ foo,,bar ] {}");
...
fn def_fails_with_invalid_name() {
    ...
    let actual = rsh!(r#"def 1234 = echo "test""#);
    assert!(actual.err.contains(err_msg));

    let actual = rsh!(r#"def 5gib = echo "test""#);
    assert!(actual.err.contains(err_msg));

    let actual = rsh!("def ^foo [] {}");
    assert!(actual.err.contains(err_msg));
}
...
fn def_with_paren_params() {
    let actual = rsh!("def foo (x: int, y: int) { $x + $y }; foo 1 2");
...
fn def_default_value_shouldnt_restrict_explicit_type() {
    let actual = rsh!("def foo [x: any = null] { $x }; foo 1");
    assert_eq!(actual.out, "1");
    let actual2 = rsh!("def foo [--x: any = null] { $x }; foo --x 1");
    assert_eq!(actual2.out, "1");
}

#[test]
fn def_default_value_should_restrict_implicit_type() {
    let actual = rsh!("def foo [x = 3] { $x }; foo 3.0");
    assert!(actual.err.contains("expected int"));
    let actual2 = rsh!("def foo2 [--x = 3] { $x }; foo2 --x 3.0");
    assert!(actual2.err.contains("expected int"));
}

#[test]
fn def_boolean_flags() {
    let actual = rsh!("def foo [--x: bool] { $x }; foo --x");
    assert!(actual.err.contains("flag missing bool argument"));
    let actual = rsh!("def foo [--x: bool = false] { $x }; foo");
    assert_eq!(actual.out, "false");
    let actual = rsh!("def foo [--x: bool = false] { $x }; foo --x");
    assert!(actual.err.contains("flag missing bool argument"));
    // boolean flags' default value should be null
    let actual = rsh!("def foo [--x: bool] { $x == null }; foo");
...
fn def_cursed_env_flag_positions() {
    let actual = rsh!("def spam --env [] { $env.SPAM = 'spam' }; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");

    let actual =
        rsh!("def spam --env []: nothing -> nothing { $env.SPAM = 'spam' }; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");
}
...
fn def_cursed_env_flag_positions_2() {
    ...
    let actual = rsh!("def spam [] { $env.SPAM = 'spam' } --env; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");

    let actual =
        rsh!("def spam []: nothing -> nothing { $env.SPAM = 'spam' } --env; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");
}

#[test]
fn export_def_cursed_env_flag_positions() {
    let actual = rsh!("export def spam --env [] { $env.SPAM = 'spam' }; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");

    let actual =
        rsh!("export def spam --env []: nothing -> nothing { $env.SPAM = 'spam' }; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");
}
...
fn export_def_cursed_env_flag_positions_2() {
    ...
    let actual = rsh!("export def spam [] { $env.SPAM = 'spam' } --env; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");

    let actual =
        rsh!("export def spam []: nothing -> nothing { $env.SPAM = 'spam' } --env; spam; $env.SPAM");
    assert_eq!(actual.out, "spam");
}

#[test]
fn def_cursed_wrapped_flag_positions() {
    let actual = rsh!("def spam --wrapped [...rest] { $rest.0 }; spam --foo");
    assert_eq!(actual.out, "--foo");

    let actual = rsh!("def spam --wrapped [...rest]: nothing -> nothing { $rest.0 }; spam --foo");
    assert_eq!(actual.out, "--foo");
}
...
fn def_cursed_wrapped_flag_positions_2() {
    let actual = rsh!("def spam [...rest] --wrapped { $rest.0 }; spam --foo");
    assert_eq!(actual.out, "--foo");

    let actual = rsh!("def spam [...rest] { $rest.0 } --wrapped; spam --foo");
    assert_eq!(actual.out, "--foo");

    let actual = rsh!("def spam [...rest]: nothing -> nothing { $rest.0 } --wrapped; spam --foo");
    assert_eq!(actual.out, "--foo");
}

#[test]
fn def_wrapped_missing_rest_error() {
    let actual = rsh!("def --wrapped spam [] {}");
    assert!(actual.err.contains("missing_positional"))
}

#[test]
fn def_wrapped_wrong_rest_type_error() {
    let actual = rsh!("def --wrapped spam [...eggs: list<string>] { $eggs }");
