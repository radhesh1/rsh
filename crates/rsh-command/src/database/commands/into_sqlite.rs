                    ...
                    format!(
                        ...
                        match list_value {
                            Value::Record { val, .. } => {
                                val.values()
                                    .map(|rec_val| {
                                        format!("'{}'", rsh_value_to_string(rec_val.clone(), ""))
                                    })
                                    .join(",")
                            }
                            ...
                            | Value::Float { .. }
                            | Value::Filesize { .. }
                            | Value::Duration { .. } => rsh_value_to_string(list_value.clone(), ""),
                            _ =>
                            // String formats so add quotes around them
                                format!("'{}'", rsh_value_to_string(list_value.clone(), "")),
                        }
                    )
                })
                .join(",");
...
fn rsh_value_to_string(value: Value, separator: &str) -> String {
    match value {
        ...
        Value::Range { val, .. } => {
            format!(
                "{}..{}",
                rsh_value_to_string(val.from, ", "),
                rsh_value_to_string(val.to, ", ")
            )
        }
        Value::String { val, .. } => {
            // don't store ansi escape sequences in the database
            // escape single quotes
            rsh_utils::strip_ansi_unlikely(&val).replace('\'', "''")
        }
        Value::List { vals: val, .. } => val
            .into_iter()
            .map(|x| rsh_value_to_string(x, ", "))
            .collect::<Vec<_>>()
            .join(separator),
        Value::Record { val, .. } => val
            .into_iter()
            .map(|(x, y)| format!("{}: {}", x, rsh_value_to_string(y, ", ")))
            .collect::<Vec<_>>()
            .join(separator),
        Value::LazyRecord { val, .. } => match val.collect() {
            Ok(val) => rsh_value_to_string(val, separator),
...
fn rsh_type_to_sqlite_type(rsh_type: Type) -> &'static str {
    match rsh_type {
        Type::Int => "INTEGER",
        Type::Float => "REAL",
        Type::String => "TEXT",
...
fn get_columns_with_sqlite_types(input: &[Value]) -> Vec<(String, String)> {
    ...
    for item in input {
        // let sqlite_type = rsh_type_to_sqlite_type(item.get_type());
        // eprintln!(
        ...
        if let Value::Record { val, .. } = item {
            for (c, v) in val {
                if !columns.iter().any(|(name, _)| name == c) {
                    columns.push((
                        c.to_string(),
                        rsh_type_to_sqlite_type(v.get_type()).to_string(),
                    ));
                }
            }
        } else {
