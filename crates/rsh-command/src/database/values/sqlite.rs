...
use rsh_protocol::{CustomValue, PipelineData, Record, ShellError, Span, Spanned, Value};
...
pub fn open_sqlite_db(path: &Path, call_span: Span) -> Result<Connection, rsh_protocol::ShellError> {
...
fn run_sql_query(
    ...
    prepared_statement_to_rsh_list(stmt, sql.span, ctrlc)
}
...
fn read_single_table(
    ...
    prepared_statement_to_rsh_list(stmt, call_span, ctrlc)
}

fn prepared_statement_to_rsh_list(
    ...
    let row_results = stmt.query_map([], |row| {
        Ok(convert_sqlite_row_to_rsh_value(
            row,
            call_span,
            column_names.clone(),
        ))
    ...
    for row_result in row_results {
        if rsh_utils::ctrl_c::was_pressed(&ctrlc) {
...
fn read_entire_sqlite_db(
    ...
    for row in rows {
        ...
        let rows = prepared_statement_to_rsh_list(table_stmt, call_span, ctrlc.clone())?;
...
pub fn convert_sqlite_row_to_rsh_value(row: &Row, span: Span, column_names: Vec<String>) -> Value {
    let mut vals = Vec::with_capacity(column_names.len());

    for i in 0..column_names.len() {
        let val = convert_sqlite_value_to_rsh_value(row.get_ref_unwrap(i), span);
        vals.push(val);
    }
...
pub fn convert_sqlite_value_to_rsh_value(value: ValueRef, span: Span) -> Value {
    match value {
