use rsh_engine::{eval_block_with_early_return, CallExt};
...
impl Command for Source {
  ...
  https://irsh.eu.org/book/thinking_in_nu.html"#
    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            Example {
                description: "Runs foo.rsh in the current context",
                example: r#"source foo.rsh"#,
                result: None,
            },
            Example {
                description: "Runs foo.rsh in current context and call the command defined, suppose foo.rsh has content: `def say-hi [] { echo 'Hi!' }`",
                example: r#"source ./foo.rsh; say-hi"#,
                result: None,
            },
        ]
    }
