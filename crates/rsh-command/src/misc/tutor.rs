...
fn default_tutor() -> &'static str {
    r#"
Welcome to the Rsh tutorial!

With the `tutor` command, you'll be able to learn a lot about how Rsh
...
fn begin_tutor() -> &'static str {
    r#"
Rsh is a structured shell and programming language. One way to begin
...
fn table_tutor() -> &'static str {
    r#"
The most common form of data in Rsh is the table. Tables contain rows and
...
Rsh. Because of this, there is a rich list of commands to work with cells
...
fn echo_tutor() -> &'static str {
    r#"
The `echo` command in Rsh is a powerful tool for not only seeing values,
...
$x
```
Rsh also comes with built-in variables. The `$rsh` variable is a reserved
variable that contains a lot of information about the currently running
instance of Rsh. The `$it` variable is the name given to block parameters
...
fn display(help: &str, engine_state: &EngineState, stack: &mut Stack, span: Span) -> PipelineData {
    ...
    for item in help {
        if code_mode {
            ...
            if let Some(highlighter) = engine_state.find_decl(b"rsh-highlight", &[]) {
