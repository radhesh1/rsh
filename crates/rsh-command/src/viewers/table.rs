...
use rsh_color_config::color_from_hex;
use rsh_color_config::{StyleComputer, TextStyle};
use rsh_engine::{env::get_config, env_to_string, CallExt};
use rsh_protocol::{
    ast::Call,
...
use rsh_protocol::{record, TableMode};
use rsh_table::common::create_rsh_table_config;
use rsh_table::{
    CollapsedTable, ExpandedTable, JustTable, RshTable, RshTableCell, StringResult, TableOpts,
    TableOutput,
};
use rsh_utils::get_ls_colors;
...
impl Command for Table {
    ...
    fn run(
        ...
        #[cfg(windows)]
        {
            let _ = rsh_utils::enable_vt_processing();
        }
...
fn report_unsuccessful_output(ctrlc1: Option<Arc<AtomicBool>>, term_width: usize) -> String {
    if rsh_utils::ctrl_c::was_pressed(&ctrlc1) {
        "".into()
    } else {
...
impl Iterator for PagingTableCreator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for item in self.stream.by_ref() {
            ...
            if rsh_utils::ctrl_c::was_pressed(&self.ctrlc) {
                break;
            }
        }
...
fn render_path_name(
    ...
    let stripped_path = rsh_utils::strip_ansi_unlikely(path);
...
fn maybe_strip_color(output: String, config: &Config) -> String {
    ...
    if !config.use_ansi_coloring || !std::io::stdout().is_terminal() {
        ...
        rsh_utils::strip_ansi_string_likely(output)
    } else {
...
fn create_empty_placeholder(
    ...
    let cell = RshTableCell::new(format!("empty {}", value_type_name));
    ...
    let config = create_rsh_table_config(&config, style_computer, &out, false, TableMode::default());
...
fn convert_table_to_output(
    ...
    match table {
        Ok(Some(table)) => {
            ...
            bytes.push(b'\n'); // rsh-table tables don't come with a newline on the end

            Some(Ok(bytes))
        }
        Ok(None) => {
            let msg = if rsh_utils::ctrl_c::was_pressed(ctrlc) {
                String::from("")
            } else {
                // assume this failed because the table was too wide
                // TODO: more robust error classification
