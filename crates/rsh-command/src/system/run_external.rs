use rsh_cmd_base::hook::eval_hook;
...
use rsh_system::ForegroundProcess;
use rsh_utils::IgnoreCaseExt;
...
impl ExternalCommand {
                ...
                Err(err) => {
                    ...
                    if looks_like_cmd_internal {
                        ...
                        #[cfg(feature = "which-support")]
                        {
                            ...
                            if let Ok(path) =
                                rsh_engine::env::path_str(engine_state, stack, self.name.span)
                            {
                    ...
                    if let Some(mut stdin_write) = child.as_mut().stdin.take() {
                        thread::Builder::new()
                            ...
                            .spawn(move || {
                                ...
                                // they need to get Rsh data rendered for display to users.
                                //
    ...
    pub fn spawn_simple_command(&self, cwd: &str) -> Result<std::process::Command, ShellError> {
        ...
        let head = rsh_path::expand_to_real_path(head)
            .to_string_lossy()
            .to_string();
...
fn trim_expand_and_apply_arg(
    ...
    // `as_a_whole` will be true, so rsh won't remove the inner quotes.
    ...
    if !keep_raw {
        arg.item = rsh_path::expand_tilde(arg.item)
            .to_string_lossy()
            .to_string();
    }
    let cwd = PathBuf::from(cwd);
    if arg.item.contains('*') && run_glob_expansion {
        if let Ok((prefix, matches)) = rsh_engine::glob_from(&arg, &cwd, arg.span, None) {
