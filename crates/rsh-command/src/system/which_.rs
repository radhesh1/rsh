...
fn which_single(
    ...
    //If prog_name is an external command, don't search for rsh-specific programs
    ...
    match (all, external) {
        ...
        (false, false) => {
            let rsh_entries = get_entries_in_nu(engine_state, &prog_name, application.span, true);
            if !rsh_entries.is_empty() {
                return vec![rsh_entries[0].clone()];
