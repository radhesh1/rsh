use rsh_engine::{find_in_dirs_env, get_dirs_var_from_call, CallExt};
use rsh_parser::{parse, parse_module_block, unescape_unquote_string};
...
impl Command for RshCheck {
    fn name(&self) -> &str {
        "rsh-check"
    }

    fn signature(&self) -> Signature {
        Signature::build("rsh-check")
    ...
    fn run(
        ...
        if is_all && is_module {
            return Err(ShellError::GenericError(
                ...
                "You cannot have both `--all` and `--as-module` on the same command line, please refer to `rsh-check --help` for more details".to_string(),
                Some(call.head),
                None, vec![]));
        }
                ...
                if let Some(path_str) = path {
                    ...
                    if ext[0] != "rsh" {
                        return Err(ShellError::GenericError(
                            ...
                            "File extension must be the type of .rsh".to_string(),
                            Some(call.head),
                            None,
                            Vec::new(),
                        ));
                    ...
                    Err(ShellError::GenericError(
                        ...
                        "Please run 'rsh-check --help' for more details".to_string(),
                        Some(call.head),
                        None,
                        Vec::new(),
                    ))
                ...
                Err(_) => {
                    if is_debug {
                        Err(ShellError::GenericError(
                            ...
                            Some(span),
                            Some("Run `rsh-check --help` for more details".to_string()),
                            Vec::new(),
                        ))
                    } else {
