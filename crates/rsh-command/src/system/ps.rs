...
impl Command for Ps {
    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            ...
            Example {
                description: "List the system processes with 'rsh' in their names",
                example: "ps | where name =~ 'rsh'",
                result: None,
            },
            Example {
                description: "Get the parent process id of the current rsh process",
                example: "ps | where pid == $rsh.pid | get ppid",
                result: None,
            },
        ]
    }
...
fn run_ps(engine_state: &EngineState, call: &Call) -> Result<PipelineData, ShellError> {
    ...
    for proc in rsh_system::collect_proc(Duration::from_millis(100), false) {
