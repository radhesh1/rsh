...
use rsh_protocol::engine::{EngineState, Stack};
use rsh_protocol::record;
use rsh_protocol::{
...
╰───┴─────┴───────────────────────────────────────╯"
    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            ...
            Example {
                ...
                example: r#"$'(ansi red_bold)Hello(ansi reset) (ansi green_dimmed)Rsh(ansi reset) (ansi purple_italic)World(ansi reset)'"#,
                result: Some(Value::test_string(
                    "\u{1b}[1;31mHello\u{1b}[0m \u{1b}[2;32mRsh\u{1b}[0m \u{1b}[3;35mWorld\u{1b}[0m",
                )),
            },
            Example {
                description: "The same example as above with short names",
                example: r#"$'(ansi rb)Hello(ansi reset) (ansi gd)Rsh(ansi reset) (ansi pi)World(ansi reset)'"#,
                result: Some(Value::test_string(
                    "\u{1b}[1;31mHello\u{1b}[0m \u{1b}[2;32mRsh\u{1b}[0m \u{1b}[3;35mWorld\u{1b}[0m",
                )),
            },
    ...
    $"(ansi --escape $bold_blue_on_red)Hello Rsh World(ansi reset)""#,
                result: Some(Value::test_string(
                    "\u{1b}[1;48;2;255;0;0;38;2;0;0;255mHello Rsh World\u{1b}[0m",
                )),
            },
        ]
    }
    ...
    fn run(
        ...
        // this record is defined in rsh-color-config crate
        ...
        let output = if escape && param_is_valid_string {
            ...
            if code_string.starts_with('#') {
                match rsh_color_config::color_from_hex(&code_string) {
            ...
            // create a RshStyle to parse the information into
            let mut nu_style = rsh_color_config::RshStyle {
                fg: None,
                bg: None,
                attr: None,
            };
            // Iterate and populate RshStyle with real values
            for (k, v) in record {
                match k.as_str() {
            ...
            // Now create a nu_ansi_term::Style from the RshStyle
            let style = rsh_color_config::parse_rsh_style(nu_style);
