...
fn action(input: &Value, _args: &CellPathOnlyArgs, _span: Span) -> Value {
    ...
    match input {
        Value::String { val, .. } => {
            Value::string(rsh_utils::strip_ansi_likely(val).to_string(), span)
        }
        other => {
...
mod tests {
    ...
    fn test_stripping() {
        let input_string =
            Value::test_string("\u{1b}[3;93;41mHello\u{1b}[0m \u{1b}[1;32mRsh \u{1b}[1;35mWorld");
        let expected = Value::test_string("Hello Rsh World");
