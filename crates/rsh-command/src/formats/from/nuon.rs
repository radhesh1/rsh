use rsh_protocol::ast::{Call, Expr, Expression, PipelineElement};
...
#[derive(Clone)]
pub struct FromRshon;

impl Command for FromRshon {
    fn name(&self) -> &str {
        "from nuon"
    }
    ...
    fn run(
        ...
        let mut block = rsh_parser::parse(&mut working_set, None, string_input.as_bytes(), false);
