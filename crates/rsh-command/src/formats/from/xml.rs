...
mod tests {
    ...
    fn parses_empty_element() -> Result<(), roxmltree::Error> {
        ...
        assert_eq!(parse(source)?, content_tag("rsh", indexmap! {}, &[]));
    ...
    fn parses_element_with_text() -> Result<(), roxmltree::Error> {
        let source = "<rsh>La era de los tres caballeros</rsh>";
...
<rsh version=\"2.0\">
        ...
        assert_eq!(
            parse(source)?,
            content_tag("rsh", indexmap! {"version" => "2.0"}, &[])
        );
...
<rsh version=\"2.0\">
    ...
    fn parses_element_with_multiple_attributes() -> Result<(), roxmltree::Error> {
        let source = "\
<rsh version=\"2.0\" age=\"25\">
</rsh>";

        assert_eq!(
            parse(source)?,
            content_tag("rsh", indexmap! {"version" => "2.0", "age" => "25"}, &[])
        );
