...
impl Command for FromJson {
    ...
    fn run(
        ...
        // TODO: turn this into a structured underline of the rsh_json error
...
fn convert_nujson_to_value(value: &rsh_json::Value, span: Span) -> Value {
    match value {
        rsh_json::Value::Array(array) => {
        ...
        rsh_json::Value::Bool(b) => Value::bool(*b, span),
        rsh_json::Value::F64(f) => Value::float(*f, span),
        rsh_json::Value::I64(i) => Value::int(*i, span),
        rsh_json::Value::Null => Value::nothing(span),
        rsh_json::Value::Object(k) => Value::record(
            k.iter()
                .map(|(k, v)| (k.clone(), convert_nujson_to_value(v, span)))
                .collect(),
            span,
        ),
        rsh_json::Value::U64(u) => {
            if *u > i64::MAX as u64 {
                Value::error(
        ...
        rsh_json::Value::String(s) => Value::string(s.clone(), span),
    }
}
...
fn convert_string_to_value(string_input: String, span: Span) -> Result<Value, ShellError> {
    let result: Result<rsh_json::Value, rsh_json::Error> = rsh_json::from_str(&string_input);
    match result {
        Ok(value) => Ok(convert_nujson_to_value(&value, span)),

        Err(x) => match x {
            rsh_json::Error::Syntax(_, row, col) => {
