...
impl Command for ConfigEnv {
    ...
    fn signature(&self) -> Signature {
        Signature::build(self.name())
            ...
            .switch("default", "Print default `env.rsh` file instead.", Some('d'))
    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            Example {
                description: "allow user to open and update rsh env",
                example: "config env",
                result: None,
            },
            Example {
                description: "allow user to print default `env.rsh` file",
    ...
    fn run(
        ...
        if call.has_flag("default") {
            let head = call.head;
            return Ok(Value::string(rsh_utils::get_default_env(), head).into_pipeline_data());
        }
        ...
        let nu_config = match engine_state.get_config_path("env-path") {
            ...
            None => {
                return Err(ShellError::GenericError(
                    "Could not find $rsh.env-path".to_string(),
                    "Could not find $rsh.env-path".to_string(),
                    None,
                    None,
                    Vec::new(),
