...
use rsh_path::expand_to_real_path;
    ...
    fn run(
        ...
        let (path, p_tag, absolute_path) = match pattern_arg {
            Some(p) => {
                ...
                let expanded = rsh_path::expand_path_with(&p, &cwd);
        ...
        let (prefix, paths) = rsh_engine::glob_from(&glob_path, &cwd, call_span, glob_options)?;
