...
use rsh_engine::{current_dir, eval_block, CallExt};
use rsh_protocol::ast::Call;
use rsh_protocol::engine::{Closure, Command, EngineState, Stack, StateWorkingSet};
use rsh_protocol::{
...
impl Command for Watch {
    ...
    fn usage(&self) -> &str {
        "Watch for file changes and execute Rsh code when they happen."
    }
    ...
    fn signature(&self) -> rsh_protocol::Signature {
        ...
        .input_output_types(vec![(Type::Nothing, Type::Table(vec![]))])
            ...
            SyntaxShape::Closure(Some(vec![SyntaxShape::String, SyntaxShape::String, SyntaxShape::String])),
                "Some Rsh code to run whenever a file changes. The closure will be passed `operation`, `path`, and `new_path` (for renames only) arguments in that order")
            .named(
                "debounce-ms",
                SyntaxShape::Int,
    ...
    fn run(
        ...
        let path = match rsh_path::canonicalize_with(path_no_whitespace, cwd) {
            Ok(p) => p,
            Err(_) => {
        ...
        let glob_pattern = match glob_flag {
            Some(glob) => {
                ...
                match rsh_glob::Pattern::new(&absolute_path.to_string_lossy()) {
        ...
        loop {
            ...
            if rsh_utils::ctrl_c::was_pressed(ctrlc_ref) {
                break;
            }
        }
