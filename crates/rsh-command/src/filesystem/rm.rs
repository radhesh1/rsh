...
use rsh_cmd_base::arg_glob_leading_dot;
use rsh_engine::env::current_dir;
...
fn rm(
    ...
    let home: Option<String> = rsh_path::home_dir().map(|path| {
        {
            if path.exists() {
                match rsh_path::canonicalize_with(&path, &currentdir_path) {
    ...
    for (idx, path) in targets.clone().into_iter().enumerate() {
        ...
        let corrected_path = Spanned {
            item: rsh_utils::strip_ansi_string_unlikely(path.item),
            span: path.span,
        };
        ...
        if rm_always_trash {
            return Err(ShellError::GenericError(
                "Cannot execute `rm`; the current configuration specifies \
                    `always_trash = true`, but the current rsh executable was not \
            ...
            return Err(ShellError::GenericError(
                "Cannot execute `rm` with option `--trash`; feature `trash-support` not enabled or on an unsupported platform"
                    .into(),
                "this option is only available if rsh is built with the `trash-support` feature and the platform supports trash"
                    .into(),
                Some(span),
                None,
                Vec::new(),
                ...
                for file in files {
                    match file {
                        Ok(f) => {
                            ...
                            all_targets
                                .entry(rsh_path::expand_path_with(f, &currentdir_path))
