#!/usr/bin/env python
# Example of using a Python script as a Rsh plugin
#
# The example uses JSON encoding but it should be a similar process using
# msgpack to move data between Rsh and the plugin. The only difference
...
#   represent the data transferred between Rsh and the plugin are kept as
...
def signatures():
    """
    Multiple signatures can be sent to Rsh. Each signature will be registered
    as a different plugin function in Rsh.
...
def process_call(plugin_call):
    ...
    # Creates a Value of type List that will be encoded and sent to Rsh
    return {
        "Value": {
            "List": {
                "vals": [
...
def plugin():
    ...
    if plugin_call == "Signature":
        ...
        # Use this error format if you want to return an error back to Rsh
        error = {
            "Error": {
