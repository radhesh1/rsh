...
use rsh_protocol::{CustomValue, ShellError, Value};
use serde::Serialize;
...
///
/// This is constructed by the main rsh engine when it receives [`PluginResponse::PluginData`]
...
pub struct PluginCustomValue {
    ...
    // sent between plugins and rsh's main engine. PluginCustomValue is never meant to be sent
...
impl CustomValue for PluginCustomValue {
    fn clone_value(&self, span: rsh_protocol::Span) -> rsh_protocol::Value {
    ...
    fn to_base_value(
        &self,
        span: rsh_protocol::Span,
    ) -> Result<rsh_protocol::Value, rsh_protocol::ShellError> {
