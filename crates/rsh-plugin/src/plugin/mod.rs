...
use rsh_engine::documentation::get_flags_section;
...
use rsh_protocol::{CustomValue, PluginSignature, ShellError, Span, Value};

use super::EvaluatedCall;

pub(crate) const OUTPUT_BUFFER_SIZE: usize = 8192;

/// Encoding scheme that defines a plugin's communication protocol with rsh
...
#[doc(hidden)] // Note: not for plugin authors / only used in rsh-parser
...
/// The basic API for a rsh plugin
///
/// This is the trait that rsh plugins must implement. The methods defined on
...
/// ```
/// # use rsh_plugin::*;
/// # use rsh_protocol::{PluginSignature, Type, Value};
/// struct HelloPlugin;
///
...
pub trait Plugin {
    ...
    /// When rsh invoked the plugin [serve_plugin] will call this method and
    /// print the serialized returned value or error to stdout, which rsh will
    /// interpret.
    ///
...
/// ```
///
/// The object that is expected to be received by rsh is the `PluginResponse` struct.
/// The `serve_plugin` function should ensure that it is encoded correctly and sent
/// to StdOut for rsh to decode and and present its result.
pub fn serve_plugin(plugin: &mut impl Plugin, encoder: impl PluginEncoder) {
    ...
    // encoding format: |  content-length  | content    |
    {
        ...
        stdout
            ...
            .expect("Failed to tell rsh my encoding");
        stdout
            .flush()
            .expect("Failed to tell rsh my encoding when flushing stdout");
    }
    ...
    match plugin_call {
        ...
        Ok(plugin_call) => {
            match plugin_call {
                // Sending the signature back to rsh to create the declaration definition
