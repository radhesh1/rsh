...
use rsh_protocol::ShellError;

/// A `PluginEncoder` that enables the plugin to communicate with rsh with MsgPack
/// serialized data.
#[derive(Clone, Debug)]
...
impl PluginEncoder for MsgPackSerializer {
    ...
    ) -> Result<(), rsh_protocol::ShellError> {
    ...
    ) -> Result<crate::protocol::PluginCall, rsh_protocol::ShellError> {
...
mod tests {
    ...
    use rsh_protocol::{PluginSignature, Span, Spanned, SyntaxShape, Value};
    ...
    fn response_round_trip_signature() {
        let signature = PluginSignature::build("rsh-plugin")
