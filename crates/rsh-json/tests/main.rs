...
fn hjson_expectations() -> PathBuf {
    let assets = rsh_test_support::fs::assets().join("rsh_json");

    rsh_path::canonicalize(assets.clone()).unwrap_or_else(|e| {
        panic!(
...
macro_rules! run_test {
    ...
    ($v: ident, $list: expr, $fix: expr) => {{
        ...
        let data: rsh_json::Result<Value> = rsh_json::from_str(&test_content);
        ...
        if !should_fail {
            ...
            let actual_hjson = rsh_json::to_string(&udata).unwrap();
