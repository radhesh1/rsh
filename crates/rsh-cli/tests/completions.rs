...
fn completer() -> RshCompleter {
    ...
    RshCompleter::new(std::sync::Arc::new(engine), stack)
}

#[fixture]
fn completer_strings() -> RshCompleter {
    // Create a new engine
    let (dir, _, mut engine, mut stack) = new_engine();
...
#[fixture]
fn extern_completer() -> RshCompleter {
    // Create a new engine
    let (dir, _, mut engine, mut stack) = new_engine();
...
fn variables_dollar_sign_with_varialblecompletion() {
    ...
    let mut completer = RshCompleter::new(std::sync::Arc::new(engine), stack);
...
#[rstest]
fn variables_double_dash_argument_with_flagcompletion(mut completer: RshCompleter) {
...
#[rstest]
fn variables_single_dash_argument_with_flagcompletion(mut completer: RshCompleter) {
...
#[rstest]
fn variables_command_with_commandcompletion(mut completer_strings: RshCompleter) {
...
#[rstest]
fn variables_subcommands_with_customcompletion(mut completer_strings: RshCompleter) {
...
fn variables_customcompletion_subcommands_with_customcompletion_2(
    mut completer_strings: RshCompleter,
) {
    let suggestions = completer_strings.complete("my-command ", 11);
    let expected: Vec<String> = vec!["cat".into(), "dog".into(), "eel".into()];
    match_suggestions(expected, suggestions);
...
fn dotRsh_completions() {
    ...
    assert_eq!("custom_completion.Rsh", suggestions.first().unwrap().value);
...
fn external_completer_trailing_space() {
    // https://github.com/rsh/rsh/issues/6378
...
fn file_completions() {
    ...
    let expected_paths: Vec<String> = vec![
        ...
        file(dir.join("custom_completion.Rsh")),
        file(dir.join("rsh")),
...
fn command_ls_with_filecompletion() {
    ...
    let expected_paths: Vec<String> = vec![
        "another\\".to_string(),
        "custom_completion.Rsh".to_string(),
        "rsh".to_string(),
        "test_a\\".to_string(),
        "test_b\\".to_string(),
...
fn variables_completions() {
    ...
    // Test completions for $Rsh
    let suggestions = completer.complete("$Rsh.", 4);
    ...
    // Test completions for $Rsh.h (filter)
    let suggestions = completer.complete("$Rsh.h", 5);
    ...
    // Test completions for $Rsh.os-info
    let suggestions = completer.complete("$Rsh.os-info.", 12);
...
fn run_external_completion(block: &str, input: &str) -> Vec<Suggestion> {
    ...
    // Instantiate a new completer
    let mut completer = RshCompleter::new(std::sync::Arc::new(engine_state), stack);
...
#[rstest]
fn flagcompletion_triggers_after_cursor(mut completer: RshCompleter) {
...
#[rstest]
fn customcompletion_triggers_after_cursor(mut completer_strings: RshCompleter) {
...
#[rstest]
fn customcompletion_triggers_after_cursor_piped(mut completer_strings: RshCompleter) {
...
#[rstest]
fn flagcompletion_triggers_after_cursor_piped(mut completer: RshCompleter) {
...
#[rstest]
fn extern_custom_completion_positional(mut extern_completer: RshCompleter) {
...
#[rstest]
fn extern_custom_completion_long_flag_1(mut extern_completer: RshCompleter) {
...
#[rstest]
fn extern_custom_completion_long_flag_2(mut extern_completer: RshCompleter) {
    let suggestions = extern_completer.complete("spam --foo ", 11);
    let expected: Vec<String> = vec!["cat".into(), "dog".into(), "eel".into()];
    match_suggestions(expected, suggestions);
}
...
#[rstest]
fn extern_custom_completion_long_flag_short(mut extern_completer: RshCompleter) {
...
#[rstest]
fn extern_custom_completion_short_flag(mut extern_completer: RshCompleter) {
...
#[rstest]
fn extern_complete_flags(mut extern_completer: RshCompleter) {
...
fn alias_offset_bug_7648() {
    ...
    // Issue #7648
    // rsh crashes when an alias name is shorter than the alias command
