...
pub fn new_engine() -> (PathBuf, String, EngineState, Stack) {
    ...
    // Add $rsh
    let rsh_const =
        create_rsh_constant(&engine_state, Span::test_data()).expect("Failed creating $rsh");
    ...
    stack.add_env_var(
        "PWD".to_string(),
        Value::string(dir_str.clone(), rsh_protocol::Span::new(0, dir_str.len())),
    );
    stack.add_env_var(
        "TEST".to_string(),
        Value::string(
            "RSH".to_string(),
            rsh_protocol::Span::new(0, dir_str.len()),
        ),
    );
    #[cfg(windows)]
    stack.add_env_var(
