use crate::{
    completions::RshCompleter,
    prompt_update,
    reedline_config::{add_menus, create_keybindings, KeybindingsMode},
    util::eval_source,
    RshHighlighter, RshValidator, RshPrompt,
};
use crossterm::cursor::SetCursorStyle;
use log::{trace, warn};
use miette::{ErrReport, IntoDiagnostic, Result};
use rsh_cmd_base::util::get_guaranteed_cwd;
use rsh_cmd_base::{hook::eval_hook, util::get_editor};
use rsh_color_config::StyleComputer;
use rsh_engine::convert_env_values;
use rsh_parser::{lex, parse, trim_quotes_str};
use rsh_protocol::{
    config::RshCursorShape,
...
pub fn evaluate_repl(
    ...
    if !std::io::stdin().is_terminal() {
        return Err(std::io::Error::new(
            ...
            "Rsh launched as a REPL, but STDIN is not a TTY; either launch in a valid terminal or provide arguments to invoke a script!",
        ))
        .into_diagnostic();
    }
    ...
    let mut rsh_prompt = RshPrompt::new();
    ...
    let temp_file = temp_dir().join(format!("{}.rsh", uuid::Uuid::new_v4()));
    ...
    // Regenerate the $rsh constant to contain the startup time and any other potential updates
    let rsh_const = create_rsh_constant(engine_state, Span::unknown())?;
    engine_state.set_variable_const_val(RSH_VARIABLE_ID, rsh_const);
    ...
    loop {
        ...
        let cursor_config = CursorConfig {
            vi_insert: map_rshcursorshape_to_cursorshape(config.cursor_shape_vi_insert),
            vi_normal: map_rshcursorshape_to_cursorshape(config.cursor_shape_vi_normal),
            emacs: map_rshcursorshape_to_cursorshape(config.cursor_shape_emacs),
        };
        perf(
        ...
        line_editor = line_editor
            ...
            .with_highlighter(Box::new(RshHighlighter {
                engine_state: engine_reference.clone(),
                config: config.clone(),
            }))
            .with_validator(Box::new(RshValidator {
                engine_state: engine_reference.clone(),
            }))
            .with_completer(Box::new(RshCompleter::new(
        ...
        let prompt = prompt_update::update_prompt(config, engine_state, stack, &mut rsh_prompt);
        perf(
            "update_prompt",
            start_time,
            file!(),
        ...
        match input {
            Ok(Signal::Success(s)) => {
                ...
                let cwd = rsh_engine::env::current_dir_str(engine_state, stack)?;
                ...
                let path = rsh_path::expand_path_with(&orig, &cwd);
                ...
                if looks_like_path(&orig) && path.is_dir() && tokens.0.len() == 1 {
                    ...
                    let (path, span) = {
                        ...
                        let path = rsh_path::canonicalize_with(path, &cwd)
                ...
                if shell_integration {
                    ...
                    if let Some(cwd) = stack.get_env_var(engine_state, "PWD") {
                        ...
                        let maybe_abbrev_path = if let Some(p) = rsh_path::home_dir() {
...
fn map_rshcursorshape_to_cursorshape(shape: RshCursorShape) -> Option<SetCursorStyle> {
    match shape {
        RshCursorShape::Block => Some(SetCursorStyle::SteadyBlock),
        RshCursorShape::UnderScore => Some(SetCursorStyle::SteadyUnderScore),
        RshCursorShape::Line => Some(SetCursorStyle::SteadyBar),
        RshCursorShape::BlinkBlock => Some(SetCursorStyle::BlinkingBlock),
        RshCursorShape::BlinkUnderScore => Some(SetCursorStyle::BlinkingUnderScore),
        RshCursorShape::BlinkLine => Some(SetCursorStyle::BlinkingBar),
        RshCursorShape::Inherit => None,
    }
}
