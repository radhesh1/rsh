use crate::completions::{
    ...
    DotRshCompletion, FileCompletion, FlagCompletion, VariableCompletion,
};
use rsh_engine::eval_block;
use rsh_parser::{flatten_expression, parse, FlatShape};
use rsh_protocol::{
    ast::PipelineElement,
                            ...
                            if pos >= flat.0.start && pos < flat.0.end {
                                ...
                                if (is_passthrough_command && flat_idx > 1) || flat_idx > 0 {
                                    if let Some(previous_expr) = flattened.get(flat_idx - 1) {
                                        ...
                                        // Completion for .rsh files
                                        if prev_expr_str == b"use" || prev_expr_str == b"source-env"
                                        {
                                            let mut completer =
                                                DotRshCompletion::new(self.engine_state.clone());
...
impl ReedlineCompleter for RshCompleter {
...
mod completer_tests {
    ...
    fn test_completion_helper() {
        ...
        // Custom additions
        let delta = {
            let working_set = rsh_protocol::engine::StateWorkingSet::new(&engine_state);
            working_set.render()
        };
        ...
        let mut completer = RshCompleter::new(engine_state.into(), Stack::new());
        let dataset = vec![
            ("sudo", false, "", Vec::new()),
