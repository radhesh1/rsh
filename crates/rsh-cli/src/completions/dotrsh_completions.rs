...
#[derive(Clone)]
pub struct DotRshCompletion {
    engine_state: Arc<EngineState>,
}

impl DotRshCompletion {
    pub fn new(engine_state: Arc<EngineState>) -> Self {
        Self { engine_state }
    }
}

impl Completer for DotRshCompletion {
    fn fetch(
        &mut self,
        _: &StateWorkingSet,
        prefix: Vec<u8>,
        ...
        let lib_dirs: Vec<String> =
            if let Some(lib_dirs) = self.engine_state.get_env_var("rsh_LIB_DIRS") {
                lib_dirs
                    .as_list()
                    .into_iter()
                    .flat_map(|it| {
        ...
        // Fetch the files filtering the ones that ends with .rsh
        ...
        let output: Vec<Suggestion> = search_dirs
            ...
            .flat_map(|it| {
                file_path_completion(span, &partial, &it, options)
                    .into_iter()
                    .filter(|it| {
                        // Different base dir, so we list the .rsh files or folders
                        if !is_current_folder {
                            it.1.ends_with(".rsh") || it.1.ends_with(SEP)
                        } else {
                            // Lib dirs, so we filter only the .rsh files
                            it.1.ends_with(".rsh")
                        }
                    })
