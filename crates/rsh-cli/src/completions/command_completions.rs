...
impl Completer for CommandCompletion {
    fn fetch(
        ...
        let commands = if matches!(self.flat_shape, rsh_parser::FlatShape::External)
            || matches!(self.flat_shape, rsh_parser::FlatShape::InternalCall(_))
    ...
    fn test_is_last_command_passthrough() {
        ...
        for (idx, ele) in commands.iter().enumerate() {
            ...
            let delta = {
                ...
                let _ = working_set.add_file("child.rsh".into(), input);
                working_set.render()
            };
