...
use crate::{menus::RshMenuCompleter, RshHelpCompleter};
use crossterm::event::{KeyCode, KeyModifiers};
use rsh_color_config::{color_record_to_rsh_style, lookup_ansi_color_style};
        ...
        $menu = match extract_value($name, $record, $span) {
            Ok(text) => {
                let style = match text {
                    ...
                    Value::Record { .. } => color_record_to_rsh_style(&text),
...
pub(crate) fn add_columnar_menu(
    ...
    match &menu.source {
        ...
        Value::Closure { val, .. } => {
            let menu_completer = RshMenuCompleter::new(
                val.block_id,
                span,
...
pub(crate) fn add_description_menu(
    ...
    match &menu.source {
        Value::Nothing { .. } => {
            let completer = Box::new(RshHelpCompleter::new(engine_state));
            Ok(line_editor.with_menu(ReedlineMenu::WithCompleter {
