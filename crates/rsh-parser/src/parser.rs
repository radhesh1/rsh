...
use rsh_engine::DIR_VAR_PARSER_INFO;
use rsh_protocol::{
    ast::{
...
pub fn parse_multispan_value(
    ...
    match shape {
        ...
        SyntaxShape::Expression => {
            ...
            // is it subexpression?
            // Not sure, but let's make it not, so the behavior is the same as previous version of rsh.
...
fn attach_parser_info_builtin(working_set: &StateWorkingSet, name: &str, call: &mut Call) {
    match name {
        "use" | "overlay use" | "source-env" | "rsh-check" => {
...
pub fn parse_variable_expr(working_set: &mut StateWorkingSet, span: Span) -> Expression {
    ...
    if contents == b"$rsh" {
        return Expression {
            expr: Expr::Var(rsh_protocol::RSH_VARIABLE_ID),
            span,
            ty: Type::Any,
            custom_completion: None,
        };
        ...
        return Expression {
            expr: Expr::Var(rsh_protocol::IN_VARIABLE_ID),
            span,
            ty: Type::Any,
            custom_completion: None,
        };
        ...
        return Expression {
            expr: Expr::Var(rsh_protocol::ENV_VARIABLE_ID),
            span,
            ty: Type::Any,
            custom_completion: None,
        };
            ...
            LiteElement::Command(_, command) if !command.parts.is_empty() => {
                    ...
                    || working_set.get_span_contents(command.parts[0]) == b"mut"
                {
                    ...
                    if new_command.parts.len() > 3 {
                        let rhs_span = rsh_protocol::span(&new_command.parts[3..]);
...
pub fn parse(
    ...
    let name = match fname {
        Some(fname) => {
            ...
            rsh_path::expand_to_real_path(fname)
                .to_string_lossy()
                .to_string()
        }
