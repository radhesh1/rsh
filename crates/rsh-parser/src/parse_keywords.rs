...
use log::trace;
use rsh_path::canonicalize_with;
use rsh_protocol::{
    ast::{
...
pub const LIB_DIRS_VAR: &str = "RSH_LIB_DIRS";
#[cfg(feature = "plugin")]
pub const PLUGIN_DIRS_VAR: &str = "RSH_PLUGIN_DIRS";
...
pub fn parse_module_file_or_dir(
    ...
    if module_path.is_dir() {
        ...
        let mod_rsh_path = module_path.clone().join("mod.rsh");

        if !(mod_rsh_path.exists() && mod_rsh_path.is_file()) {
            working_set.error(ParseError::ModuleMissingModRshFile(
        ...
        for entry_path in dir_contents {
            if (entry_path.is_file()
                && entry_path.extension() == Some(OsStr::new("rsh"))
                && entry_path.file_stem() != Some(OsStr::new("mod")))
                || (entry_path.is_dir() && entry_path.clone().join("mod.rsh").exists())
            {
    ...
    if let Some(decl_id) = working_set.find_decl(b"let") {
        if spans.len() >= 4 {
            ...
            for span in spans.iter().enumerate() {
                ...
                // https://github.com/radhesh1/rsh/issues/9596, let = if $
                // let x = 'f', = at least start from index 2
                if item == b"=" && spans.len() > (span.0 + 1) && span.0 > 1 {
                    let (tokens, parse_error) = lex(
                        working_set.get_span_contents(rsh_protocol::span(&spans[(span.0 + 1)..])),
                    ...
                    let rvalue_span = rsh_protocol::span(&spans[(span.0 + 1)..]);
                    ...
                    if ["in", "rsh", "env"].contains(&var_name.as_str()) {
                    ...
                    if let Some(explicit_type) = &explicit_type {
                        if !type_compatible(explicit_type, &rhs_type) {
                            working_set.error(ParseError::TypeMismatch(
                                explicit_type.clone(),
                                rhs_type.clone(),
                                rsh_protocol::span(&spans[(span.0 + 1)..]),
                            ));
                        }
                    }
            ...
            if spans.len() >= 2 {
                ...
                if let Some(path) = find_in_dirs(&filename, working_set, &cwd, LIB_DIRS_VAR) {
                    if let Some(contents) = path.read(working_set) {
                        ...
                        // after writing `source example.rsh`
...
pub fn parse_register(working_set: &mut StateWorkingSet, spans: &[Span]) -> Pipeline {
    use rsh_plugin::{get_signature, PluginDeclaration};
    use rsh_protocol::{engine::Stack, PluginSignature};
    ...
    let stack = Stack::new();
    let current_envs =
        rsh_engine::env::env_to_strings(working_set.permanent_state, &stack).unwrap_or_default();

    let error = arguments.and_then(|(path, path_span)| {
        let path = path.path_buf();
        // restrict plugin file name starts with `rsh_plugin_`
        let valid_plugin_name = path
            .file_name()
            .map(|s| s.to_string_lossy().starts_with("rsh_plugin_"));

        let Some(true) = valid_plugin_name else {
            return Err(ParseError::LabeledError(
                "Register plugin failed".into(),
                "plugin name must start with rsh_plugin_".into(),
                path_span,
            ));
        };
        ...
        let signatures = signature.map_or_else(
            || {
                ...
                if signatures.is_ok() {
                    ...
                    // and not when we evaluate plugin.rsh on shell startup
...
pub fn find_in_dirs(
    ...
    pub fn find_in_dirs_with_id(
        ...
        // Look up relative path from rsh_LIB_DIRS
        working_set
    ...
    // Same as find_in_dirs_with_id but using $env.rsh_LIB_DIRS instead of constant
