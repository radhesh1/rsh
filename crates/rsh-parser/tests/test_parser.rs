use rsh_parser::*;
use rsh_protocol::ast::{Argument, Call, PathMember};
...
mod string {
    ...
    pub fn parse_string() {
        ...
        let block = parse(&mut working_set, None, b"\"hello rsh\"", true);
        ...
        assert_eq!(expressions.len(), 1);
        if let PipelineElement::Expression(_, expr) = &expressions[0] {
            assert_eq!(expr.expr, Expr::String("hello rsh".to_string()))
        } else {
            panic!("Not an expression")
        }
    }
...
mod range {
    use super::*;
    use rsh_protocol::ast::{RangeInclusion, RangeOperator};
...
mod input_types {
    ...
    use rsh_protocol::{ast::Argument, Category, PipelineData, ShellError, Type};
