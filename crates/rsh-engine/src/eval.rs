...
pub fn eval_call(
    ...
    if rsh_utils::ctrl_c::was_pressed(&engine_state.ctrlc) {
                    ...
                    match &lhs.expr {
                        ...
                        Expr::FullCellPath(cell_path) => {
                            match &cell_path.head.expr {
                                Expr::Var(var_id) | Expr::VarDecl(var_id) => {
                                    // The $env variable is considered "mutable" in rsh.
        ...
        PipelineElement::Redirection(span, redirection, expr) => {
            match &expr.expr {
                ...
                | Expr::Filepath(_) => {
                    ...
                    // when rsh get Stderr Redirection, we want to take `stdout` part of `input`
                        ...
                        match out_stream {
                            None => {
                                eval_call(engine_state, stack, &save_call, input).map(|_| {
                                    ...
                                    // So rsh handles exit_code correctly
                                    (
                                        PipelineData::ExternalStream {
                                            stdout: None,
                                            stderr: None,
                            ...
                            Some(out_stream) => {
                                ...
                                // so rsh won't hang if external command generates both too much
        ...
        for (idx, element) in elements.iter().enumerate() {
            ...
            match (eval_result, redirect_stderr) {
                ...
                (output, false) => {
                    ...
                    // don't return `Err(ShellError)`, so rsh wouldn't show extra error message.
                    if output.1 {
                        return Ok(input);
                    }
                }
