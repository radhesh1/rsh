...
use rsh_glob::MatchOptions;
use rsh_path::{canonicalize_with, expand_path_with};
use rsh_protocol::{ShellError, Span, Spanned};

const GLOB_CHARS: &[char] = &['*', '?', '['];

/// This function is like `rsh_glob::glob` from the `glob` crate, except it is relative to a given cwd.
///
...
> {
    let (prefix, pattern) = if pattern.item.contains(GLOB_CHARS) {
        ...
        let escaped_prefix = PathBuf::from(rsh_glob::Pattern::escape(&path.to_string_lossy()));
    ...
    let glob = rsh_glob::glob_with(&pattern, glob_options).map_err(|err| {
        rsh_protocol::ShellError::GenericError(
    ...
    Ok((
        ...
        Box::new(glob.map(move |x| match x {
            Ok(v) => Ok(v),
            Err(err) => Err(rsh_protocol::ShellError::GenericError(
                "Error extracting glob pattern".into(),
                err.to_string(),
                Some(span),
                None,
