use rsh_protocol::ast::{Argument, Expr, Expression};
use rsh_protocol::{
    ast::Call,
...
// Utility returns rsh-highlighted string
fn rsh_highlight_string(code_string: &str, engine_state: &EngineState, stack: &mut Stack) -> String {
...
fn get_documentation(
    ...
    for example in examples {
        ...
        if config.no_color {
            let _ = write!(long_desc, "\n  > {}\n", example.example);
        } else if let Some(highlighter) = engine_state.find_decl(b"rsh-highlight", &[]) {
            let decl = engine_state.get_decl(highlighter);
    ...
    if config.no_color {
        rsh_utils::strip_ansi_string_likely(long_desc)
    } else {
        long_desc
    }
}
...
pub fn get_flags_section<F>(
    ...
    signature: &Signature,
    mut value_formatter: F, // format default Value (because some calls cant access config or rsh-highlight)
) -> String
where
    F: FnMut(&rsh_protocol::Value) -> String,
{
    //todo make these configurable -- pull from enginestate.config
    let help_section_name: String;
    let help_subcolor_one: String;
    ...
    // For example, in rsh-plugin. In that case, we fall back on default values
