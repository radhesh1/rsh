...
use rsh_protocol::ast::{Call, Expr};
use rsh_protocol::engine::{EngineState, Stack, StateWorkingSet, PWD_ENV};
use rsh_protocol::{Config, PipelineData, ShellError, Span, Value, VarId};
...
/// case the user defined custom env conversions in config.rsh.
///
...
pub fn convert_env_values(engine_state: &mut EngineState, stack: &Stack) -> Option<ShellError> {
    ...
    if let Ok(last_overlay_name) = &stack.last_overlay_name() {
        if let Some(env_vars) = engine_state.env_vars.get_mut(last_overlay_name) {
            ...
            error = error.or_else(|| {
                Some(ShellError::RshFailedHelp { msg: "Last active overlay not found in permanent state.".into(), help: "This error happened during the conversion of environment variables from strings to rsh values.".into() })
            });
        }
    } else {
        error = error.or_else(|| {
            Some(ShellError::RshFailedHelp { msg: "Last active overlay not found in stack.".into(), help: "This error happened during the conversion of environment variables from strings to rsh values.".into() })
        });
    }
...
///
/// Then, if the file is not found in the actual cwd, RSH_LIB_DIRS is checked.
/// If there is a relative path in RSH_LIB_DIRS, it is assumed to be relative to the actual cwd
...
pub fn find_in_dirs_env(
    ...
    let lib_dirs_fallback = stack.get_env_var(engine_state, "RSH_LIB_DIRS");
