...
use rsh_cmd_base::formats::to::delimited::merge_descriptors;
...
fn get_theme_from_asset_file(
    ...
    let theme_name = match theme {
        Some(s) => &s.item,
        None => "default", // There is no theme named "default" so this will be HtmlTheme::default(), which is "rsh_default".
    };
...
fn get_html_themes(json_name: &str) -> Result<HtmlThemes, Box<dyn Error>> {
    match Assets::get(json_name) {
        Some(content) => Ok(rsh_json::from_slice(&content.data)?),
...
fn html_value(value: Value, config: &Config) -> String {
    ...
    match value {
        Value::Binary { val, .. } => {
            let output = rsh_pretty_hex::pretty_hex(&val);
