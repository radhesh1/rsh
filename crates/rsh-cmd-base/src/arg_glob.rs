...
use rsh_glob::{glob_with_parent, MatchOptions, Paths};
use rsh_protocol::{ShellError, Spanned};
use std::fs;
...
fn arg_glob_opt(
    ...
    let pattern = {
        Spanned {
            item: rsh_utils::strip_ansi_string_unlikely(pattern.item.clone()),
            span: pattern.span,
        }
    };
...
mod test {
    use super::*;
    use rsh_glob::GlobResult;
    use rsh_protocol::{Span, Spanned};
