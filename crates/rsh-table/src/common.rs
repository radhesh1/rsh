...
use rsh_protocol::{Config, FooterMode, ShellError, Span, Value};
use rsh_protocol::{TableMode, TrimStrategy};

use crate::{
    clean_charset, colorize_space_str, string_wrap, RshTableConfig, TableOutput, TableTheme,
};
...
pub fn create_rsh_table_config(
    config: &Config,
    comp: &StyleComputer,
    out: &TableOutput,
    expand: bool,
...
pub fn rsh_value_to_string_colored(val: &Value, cfg: &Config, style: &StyleComputer) -> String {
    let (mut text, value_style) = rsh_value_to_string(val, cfg, style);
...
pub fn rsh_value_to_string(val: &Value, cfg: &Config, style: &StyleComputer) -> RshText {
    let float_precision = cfg.float_precision as usize;
    let text = val.into_abbreviated_string(cfg);
    make_styled_string(style, text, Some(val), float_precision)
}

pub fn rsh_value_to_string_clean(val: &Value, cfg: &Config, style_comp: &StyleComputer) -> RshText {
    let (text, style) = rsh_value_to_string(val, cfg, style_comp);
...
pub fn get_value_style(value: &Value, config: &Config, style_computer: &StyleComputer) -> RshText {
    match value {
...
pub fn get_empty_style(style_computer: &StyleComputer) -> RshText {
    (
        String::from("❎"),
        TextStyle::with_style(
            Alignment::Right,
