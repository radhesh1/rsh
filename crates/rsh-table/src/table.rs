...
use rsh_utils::strip_ansi_unlikely;
use std::cmp::min;
...
pub type RshRecords = VecRecords<RshTableCell>;
pub type RshTableCell = CellInfo<String>;
...
impl RshTable {
    ...
    pub fn get_records_mut(&mut self) -> &mut RshRecords {
        &mut self.data
    }
    ...
    pub fn draw(self, config: RshTableConfig, termwidth: usize) -> Option<String> {
        build_table(
            self.data,
            config,
            self.alignments,
    ...
    pub fn total_width(&self, config: &RshTableConfig) -> usize {
...
impl From<Vec<Vec<CellInfo<String>>>> for RshTable {
    fn from(value: Vec<Vec<CellInfo<String>>>) -> Self {
        let mut rshtable = Self::new(0, 0);
        rshtable.data = VecRecords::new(value);
...
impl Default for RshTableConfig {
    fn default() -> Self {
        Self {
...
impl TableWidthCtrl {
    fn new(width: Vec<usize>, cfg: RshTableConfig, max: usize) -> Self {
        Self { width, cfg, max }
    }
}

impl TableOption<RshRecords, CompleteDimensionVecRecords<'_>, ColoredConfig> for TableWidthCtrl {
    fn change(
        self,
        rec: &mut RshRecords,
...
impl TableOption<RshRecords, CompleteDimensionVecRecords<'_>, ColoredConfig> for TableTrim {
    fn change(
        self,
        recs: &mut RshRecords,
        cfg: &mut ColoredConfig,
...
fn push_empty_column(data: &mut RshRecords) {
...
fn duplicate_row(data: &mut RshRecords, row: usize) {
    let records = std::mem::take(data);
    let mut inner: Vec<Vec<_>> = records.into();
...
fn truncate_columns(data: &mut RshRecords, count: usize) {
    let records = std::mem::take(data);
    let mut inner: Vec<Vec<_>> = records.into();
...
fn convert_alignment(alignment: rsh_color_config::Alignment) -> AlignmentHorizontal {
    match alignment {
        rsh_color_config::Alignment::Center => AlignmentHorizontal::Center,
        rsh_color_config::Alignment::Left => AlignmentHorizontal::Left,
        rsh_color_config::Alignment::Right => AlignmentHorizontal::Right,
    }
}
...
fn build_width(records: &RshRecords, pad: usize) -> Vec<usize> {
...
impl TableOption<RshRecords, CompleteDimensionVecRecords<'_>, ColoredConfig> for HeaderMove {
    fn change(
        self,
        recs: &mut RshRecords,
        cfg: &mut ColoredConfig,
...
fn remove_row(recs: &mut RshRecords, row: usize) -> Vec<String> {
    let count_columns = recs.count_columns();
...
impl TableOption<RshRecords, CompleteDimensionVecRecords<'_>, ColoredConfig> for StripColorFromRow {
    fn change(
        self,
        recs: &mut RshRecords,
        _: &mut ColoredConfig,
