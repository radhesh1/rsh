...
use rsh_color_config::{Alignment, StyleComputer, TextStyle};
use rsh_engine::column::get_columns;
use rsh_protocol::{Config, Record, ShellError, Span, Value};
use tabled::grid::config::Position;

use crate::{
    common::{
        create_rsh_table_config, error_sign, get_header_style, get_index_style, load_theme,
        rsh_value_to_string, rsh_value_to_string_clean, rsh_value_to_string_colored, wrap_text,
        RshText, StringResult, TableResult, INDEX_COLUMN_NAME,
    },
    string_width,
    types::has_index,
    RshTable, RshTableCell, TableOpts, TableOutput,
};
...
impl ExpandedTable {
    ...
    pub fn build_value(self, item: &Value, opts: TableOpts<'_>) -> RshText {
...
fn expanded_table_list(input: &[Value], cfg: Cfg<'_>) -> TableResult {
    ...
    if with_index {
        if with_header {
            data[0].push(RshTableCell::exact(String::from("#"), 1, vec![]));
        }

        for (row, item) in input.iter().enumerate() {
            if rsh_utils::ctrl_c::was_pressed(&cfg.opts.ctrlc) {
                return Ok(None);
            }
            ...
            let value = RshTableCell::new(text);
            data[row].push(value);
        }
    ...
    if !with_header {
        ...
        let mut table = RshTable::from(data);
    ...
    for (col, header) in headers.into_iter().enumerate() {
        ...
        let head_cell = RshTableCell::new(header);
    ...
    if truncate {
        ...
        if !is_last_column {
            let shift = RshTableCell::exact(String::from("..."), 3, vec![]);
            for row in &mut data {
...
fn expanded_table_kv(record: &Record, cfg: Cfg<'_>) -> StringResult {
    ...
    for (key, value) in record {
        ...
        let key = RshTableCell::new(key);
        let val = RshTableCell::new(value);
        let row = vec![key, val];
...
fn expanded_table_entry(item: &Value, header: &str, cfg: Cfg<'_>) -> RshText {
    match item {
...
fn expanded_table_entry2(item: &Value, cfg: Cfg<'_>) -> RshText {
    let is_limit_reached = matches!(cfg.format.expand_limit, Some(0));
    if is_limit_reached {
        return rsh_value_to_string_clean(item, cfg.opts.config, cfg.opts.style_computer);
    }
    ...
    match &item {
        Value::Record { val: record, .. } => {
            if record.is_empty() {
                return rsh_value_to_string(item, cfg.opts.config, cfg.opts.style_computer);
            }
            ...
            match table {
                ...
                _ => rsh_value_to_string(item, cfg.opts.config, cfg.opts.style_computer),
            }
        }
        Value::List { vals, .. } => {
            ...
            let out = match table {
                Ok(Some(out)) => out,
                _ => return rsh_value_to_string(item, cfg.opts.config, cfg.opts.style_computer),
            };
            ...
            match table {
                ...
                None => rsh_value_to_string(item, cfg.opts.config, cfg.opts.style_computer),
            }
        }
        _ => rsh_value_to_string_clean(item, cfg.opts.config, cfg.opts.style_computer),
    }
}
...
fn value_list_to_string(
    ...
    for (i, value) in vals.iter().enumerate() {
        ...
        let text = rsh_value_to_string_clean(value, config, style_computer).0;
        buf.push_str(&text);
    }
...
fn maybe_expand_table(out: TableOutput, term_width: usize, opts: &TableOpts<'_>) -> StringResult {
    let mut table_config =
        create_rsh_table_config(opts.config, opts.style_computer, &out, false, opts.mode);
...
fn set_data_styles(table: &mut RshTable, styles: HashMap<Position, TextStyle>) {
    for (pos, style) in styles {
        table.insert_style(pos, style);
    }
}

fn create_table_cfg(cfg: &Cfg<'_>, out: &TableOutput) -> crate::RshTableConfig {
    create_rsh_table_config(
        cfg.opts.config,
        cfg.opts.style_computer,
        out,
        false,
...
fn value_to_string(value: &Value, cfg: &Cfg<'_>) -> String {
    rsh_value_to_string(value, cfg.opts.config, cfg.opts.style_computer).0
}

fn value_to_string_clean(value: &Value, cfg: &Cfg<'_>) -> String {
    rsh_value_to_string_clean(value, cfg.opts.config, cfg.opts.style_computer).0
}
...
fn value_to_wrapped_string_clean(value: &Value, cfg: &Cfg<'_>, value_width: usize) -> String {
    let text = rsh_value_to_string_colored(value, cfg.opts.config, cfg.opts.style_computer);
