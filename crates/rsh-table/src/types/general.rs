...
use rsh_protocol::{Config, Record, ShellError, Value};

use crate::{
    clean_charset, colorize_space,
    common::{
        create_rsh_table_config, get_empty_style, get_header_style, get_index_style,
        get_value_style, rsh_value_to_string_colored, RshText, INDEX_COLUMN_NAME,
    },
    RshTable, RshTableCell, StringResult, TableOpts, TableOutput, TableResult,
};
...
fn kv_table(record: &Record, opts: TableOpts<'_>) -> StringResult {
    ...
    for ((column, value), row) in record.iter().zip(data.iter_mut()) {
        if rsh_utils::ctrl_c::was_pressed(&opts.ctrlc) {
            return Ok(None);
        }

        let value = rsh_value_to_string_colored(value, opts.config, opts.style_computer);

        let key = RshTableCell::new(column.to_string());
        let value = RshTableCell::new(value);
...
fn to_table_with_header(
    ...
    row_offset: usize,
    opts: &TableOpts<'_>,
) -> Result<Option<RshTable>, ShellError> {
    let count_rows = input.len() + 1;
    let count_columns = headers.len();
    let mut table = RshTable::new(count_rows, count_columns);
...
fn to_table_with_no_header(
    ...
    let mut table = RshTable::new(input.len(), with_index as usize + 1);
    table.set_index_style(get_index_style(opts.style_computer));
...
fn get_string_value_with_header(item: &Value, header: &str, opts: &TableOpts) -> RshText {
    match item {
...
fn get_string_value(item: &Value, opts: &TableOpts) -> RshText {
