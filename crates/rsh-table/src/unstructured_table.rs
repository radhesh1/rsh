...
use rsh_protocol::{Config, Record, Span, Value};
use tabled::{
    grid::{
...
/// UnstructuredTable has a recursive table representation of rsh_protocol::Value.
///
...
impl UnstructuredTable {
    pub fn new(value: Value, config: &Config) -> Self {
        let value = convert_rsh_value_to_table_value(value, config);
        Self { value }
    }
...
fn convert_rsh_value_to_table_value(value: Value, config: &Config) -> TableValue {
    match value {
...
fn build_vertical_map(record: Record, config: &Config) -> TableValue {
    ...
    for (key, value) in record {
        let val = convert_rsh_value_to_table_value(value, config);
...
fn build_vertical_array(vals: Vec<Value>, config: &Config) -> TableValue {
    let map = vals
        .into_iter()
        .map(|val| convert_rsh_value_to_table_value(val, config))
        .collect::<Vec<_>>();
        ...
        match val {
            Value::Record { val, .. } => {
                for (i, (_key, val)) in val.into_iter().take(count_columns).enumerate() {
                    let cell = convert_rsh_value_to_table_value(val, config);
                    list[i].push(cell);
                }
            }
            _ => unreachable!(),
