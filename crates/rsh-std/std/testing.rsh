...
def run-test [
    ...
    let rendered_module_path = ({parent: ($test.file|path dirname), stem: $test_file_name, extension: rsh}| path join)
...
"
    ...
    let result = (
        ^$rsh.current-exe --no-config-file -c $"use ($rendered_module_path) *; ($test_function_name)|to nuon"
        | complete
    )
...
export def run-tests [
    ...
    --threads: int@"rsh-complete threads", # Amount of threads to use for parallel execution. Default: All threads are utilized
] {

    let available_threads = (sys | get cpu | length)

    # Can't use pattern matching here due to https://github.com/radhesh1/rsh/issues/9198
