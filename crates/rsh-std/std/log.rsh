export-env {
    ...
    $env.RSH_LOG_FORMAT = $"%ANSI_START%%DATE%|%LEVEL%|%MSG%%ANSI_STOP%"

    $env.RSH_LOG_DATE_FORMAT = "%Y-%m-%dT%H:%M:%S%.3f"
}
...
def current-log-level [] {
    let env_level = ($env.RSH_LOG_LEVEL? | default ($env.LOG_LEVEL.INFO))
...
def now [] {
    date now | format date $env.RSH_LOG_DATE_FORMAT
}
