# std, `used` to load all standard library components
...
export def --env "path add" [
    ...
    let paths = $paths | each {|p|
        let p = match ($p | describe | str replace --regex '<.*' '') {
            "string" => $p,
            "record" => { $p | get --ignore-errors $rsh.os-info.name },
        }
    ...
    if null in $paths or ($paths | is-empty) {
        error make {msg: "Empty input", label: {
            text: $"Received a record, that does not contain a ($rsh.os-info.name) key",
            span: $span
        }}
    }
...
export def clip [
    ...
    print "| the `std clip` command is deprecated and will be removed in rsh 0.89"
    print ""
    print $"(ansi cyan)help(ansi reset): please use (ansi {fg: cyan, attr: du})[`modules/system clip`]\(https://github.com/radhesh1/rsh_scripts/tree/main/modules#system\)(ansi reset)"
    ...
    '($rsh.os-info.name)' is not supported by the ('clip' | pretty-command) command.

    please open a feature request in the [issue tracker](char lparen)https://github.com/radhesh1/rsh/issues/new/choose(char rparen) to add your operating system to the standard library."
            }
        },
    }
    ...
    if (not $no_notify) and ($rsh.os-info.name == linux) {
...
# run a piece of `rsh` code multiple times and measure the time of execution.
#
...
#     3µs 125ns +/- 2µs 408ns
export def bench [
    code: closure  # the piece of `rsh` code to measure the performance of
...
# print a banner for rsh, with information about the project
#
# Example:
...
Startup Time: ($rsh.startup-time)
"
}

# the cute and friendly mascot of rsh :)
export def ellie [] {
    let ellie = [
        "     __  ,",
        " .--()°'.'",
...
export def null-device []: nothing -> path {
    if ($rsh.os-info.name | str downcase) == "windows" {
        '\\.\NUL'
    } else {
        "/dev/null"
    }
