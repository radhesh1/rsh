...
def run-command [
    ...
    do {
        if ($level_prefix | is-empty) {
            if ($ansi | is-empty) {
                ^$rsh.current-exe --commands $'use std; RSH_LOG_LEVEL=($system_level) std log custom "($message)" "($format)" ($log_level)'
            } else {
                ^$rsh.current-exe --commands $'use std; RSH_LOG_LEVEL=($system_level) std log custom "($message)" "($format)" ($log_level) --ansi "($ansi)"'
            }
        } else {
            ^$rsh.current-exe --commands $'use std; RSH_LOG_LEVEL=($system_level) std log custom "($message)" "($format)" ($log_level) --level-prefix "($level_prefix)" --ansi "($ansi)"'
        }
