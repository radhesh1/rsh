...
def run [
    ...
    do {
        if $short {
            ^$rsh.current-exe --commands $'use std; RSH_LOG_LEVEL=($system_level) std log ($message_level) --short "test message"'
        } else {
            ^$rsh.current-exe --commands $'use std; RSH_LOG_LEVEL=($system_level) std log ($message_level) "test message"'
        }
