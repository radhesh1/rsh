...
def run-command [
    ...
    do {
        if $short {
            ^$rsh.current-exe --commands $'use std; RSH_LOG_LEVEL=($system_level) std log ($message_level) --format "($format)" --short "($message)"'
        } else {
            ^$rsh.current-exe --commands $'use std; RSH_LOG_LEVEL=($system_level) std log ($message_level) --format "($format)" "($message)"'
        }
