    ...
    fn examples(&self) -> Vec<Example> {
        ...
        vec![
            Example {
                ...
                example: "[[a b]; [1 2] [3 4]] | dfr into-df | dfr into-rsh",
                result: Some(Value::list(vec![rec_1, rec_2], Span::test_data())),
            },
            Example {
                description: "Shows tail rows from dataframe",
                example: "[[a b]; [1 2] [5 6] [3 4]] | dfr into-df | dfr into-rsh --tail --rows 1",
                result: Some(Value::list(vec![rec_3], Span::test_data())),
            },
            Example {
                description: "Convert a col expression into a rsh value",
                example: "dfr col a | dfr into-rsh",
...
mod test {
    ...
    fn test_examples_dataframe_input() {
        test_dataframe(vec![Box::new(ToRsh {})])
    }

    #[test]
    fn test_examples_expression_input() {
        test_dataframe(vec![Box::new(ToRsh {}), Box::new(ExprCol {})])
    }
}
