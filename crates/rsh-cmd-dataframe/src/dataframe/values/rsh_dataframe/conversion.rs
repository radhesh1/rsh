...
use rsh_protocol::{Record, ShellError, Span, Value};

use super::{DataFrameValue, rshDataFrame};
...
// between rsh tables the values shouldn't be too extremely large for
...
fn series_to_values(
    ...
    match series.dtype() {
        ...
        e => Err(ShellError::GenericError(
            ...
            "".to_string(),
            None,
            Some(format!("Value not supported in rsh: {e}")),
            Vec::new(),
        )),
    }
}
