use rsh_protocol::{ast::Operator, ShellError, Span, Spanned, Value};
    ...
    pub fn compute_with_value(
        ...
        match right {
            Value::CustomValue { val: rhs, .. } => {
                let rhs = rhs.as_any().downcast_ref::<rshDataFrame>().ok_or_else(|| {
                ...
                match (self.is_series(), rhs.is_series()) {
                    (true, true) => {
                        ...
                        compute_between_series(
                            op,
                            &rshDataFrame::default_value(lhs_span),
                            lhs,
                            right,
                            rhs,
                        )
            ...
            _ => {
                ...
                compute_series_single_value(op, &rshDataFrame::default_value(lhs_span), self, right)
            }
        }
    }
    ...
    pub fn append_df(
        ...
        match axis {
            Axis::Row => {
                ...
                Ok(rshDataFrame::new(false, df_new))
            }
            Axis::Column => {
