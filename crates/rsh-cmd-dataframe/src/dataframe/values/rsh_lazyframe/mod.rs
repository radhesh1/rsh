...
use super::{rshDataFrame, rshExpression};
use core::fmt;
...
impl Serialize for rshLazyFrame {
...
impl<'de> Deserialize<'de> for rshLazyFrame {
    fn deserialize<D>(_deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        Ok(rshLazyFrame::default())
    }
}

impl fmt::Debug for rshLazyFrame {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "rshLazyframe")
    }
}

// Referenced access to the real LazyFrame
impl AsRef<LazyFrame> for rshLazyFrame {
...
impl AsMut<LazyFrame> for rshLazyFrame {
...
impl From<LazyFrame> for rshLazyFrame {
...
impl rshLazyFrame {
    ...
    pub fn from_dataframe(df: rshDataFrame) -> Self {
    ...
    pub fn collect(self, span: Span) -> Result<rshDataFrame, ShellError> {
        self.lazy
    ...
    pub fn try_from_value(value: Value) -> Result<Self, ShellError> {
        if Self::can_downcast(&value) {
            ...
            Ok(rshLazyFrame::from_dataframe(df))
        } else {
    ...
    pub fn apply_with_expr<F>(self, expr: rshExpression, f: F) -> Self
    where
