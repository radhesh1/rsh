...
impl CustomValue for rshExpression {
    ...
    fn clone_value(&self, span: rsh_protocol::Span) -> Value {
        let cloned = rshExpression(self.0.clone());
...
fn compute_with_value(
    ...
    match right {
        Value::CustomValue { val: rhs, .. } => {
            let rhs = rhs.as_any().downcast_ref::<rshExpression>().ok_or_else(|| {
        ...
        _ => {
            let rhs = rshExpression::try_from_value(right.clone())?;
            with_operator(operator, left, &rhs, lhs_span, right.span(), op)
        }
    }
}
...
where
    F: Fn(Expr, Expr) -> Expr,
{
    let expr: rshExpression = f(left.as_ref().clone(), right.as_ref().clone()).into();
