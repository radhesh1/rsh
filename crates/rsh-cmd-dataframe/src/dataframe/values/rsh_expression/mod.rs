...
use rsh_protocol::{record, PipelineData, ShellError, Span, Value};
use polars::prelude::{col, AggExpr, Expr, Literal};
use serde::{Deserialize, Deserializer, Serialize, Serializer};

// Polars Expression wrapper for rsh operations
// Object is behind and Option to allow easy implementation of
// the Deserialize trait
#[derive(Default, Clone, Debug)]
pub struct rshExpression(Option<Expr>);

// Mocked serialization of the LazyFrame object
impl Serialize for rshExpression {
...
impl<'de> Deserialize<'de> for rshExpression {
    fn deserialize<D>(_deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        Ok(rshExpression::default())
    }
}

// Referenced access to the real LazyFrame
impl AsRef<Expr> for rshExpression {
...
impl AsMut<Expr> for rshExpression {
...
impl From<Expr> for rshExpression {
...
impl rshExpression {
    ...
    pub fn try_from_value(value: Value) -> Result<Self, ShellError> {
        ...
        match value {
            Value::CustomValue { val, .. } => match val.as_any().downcast_ref::<Self>() {
                Some(expr) => Ok(rshExpression(expr.0.clone())),
    ...
    pub fn apply_with_expr<F>(self, other: rshExpression, f: F) -> Self
    where
        F: Fn(Expr, Expr) -> Expr,
    {
    ...
    // Convenient function to extract multiple Expr that could be inside a rsh Value
...
impl ExtractedExpr {
    ...
    fn extract_exprs(value: Value) -> Result<ExtractedExpr, ShellError> {
        match value {
            ...
            Value::CustomValue { .. } => rshExpression::try_from_value(value)
                .map(rshExpression::into_polars)
...
pub fn expr_to_value(expr: &Expr, span: Span) -> Result<Value, ShellError> {
    match expr {
        ...
        Expr::Selector(_) => Err(ShellError::UnsupportedInput {
            msg: "Expressions of type Selector to rsh Values is not yet supported".to_string(),
