use crate::dataframe::values::{Column, rshDataFrame, rshExpression, rshWhen};
...
impl Command for ExprWhen {
   ...
   | dfr collect"#,
    ...
    fn run(
        ...
        let when_predicate = rshExpression::try_from_value(when_predicate)?;

        let then_predicate: Value = call.req(engine_state, stack, 1)?;
        let then_predicate = rshExpression::try_from_value(then_predicate)?;

        let value = input.into_value(call.head);
        let when_then: rshWhen = match value {
            Value::Nothing { .. } => when(when_predicate.into_polars())
                .then(then_predicate.into_polars())
                .into(),
            v => match rshWhen::try_from_value(v)? {
                rshWhen::Then(when_then) => when_then
                    .when(when_predicate.into_polars())
                    .then(then_predicate.into_polars())
                    .into(),
                rshWhen::ChainedThen(when_then_then) => when_then_then
                    .when(when_predicate.into_polars())
                    .then(then_predicate.into_polars())
                    .into(),
            },
