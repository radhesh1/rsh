    ...
    ($command: ident, $name: expr, $desc: expr, $examples: expr, $func: ident, $test: ident) => {
        ...
        impl Command for $command {
            ...
            fn run(
                ...
                let expr = rshExpression::try_from_pipeline(input, call.head)?;
                let expr: rshExpression = expr.into_polars().$func().into();
    ...
    ($command: ident, $name: expr, $desc: expr, $examples: expr, $func: ident, $test: ident, $ddof: expr) => {
        ...
        impl Command for $command {
            ...
            fn run(
                ...
                let expr = rshExpression::try_from_pipeline(input, call.head)?;
                let expr: rshExpression = expr.into_polars().$func($ddof).into();
    ...
    ($command: ident, $name: expr, $desc: expr, $examples: expr, $func: ident, $test: ident, $ddof: expr) => {
        ...
        impl Command for $command {
            ...
            fn run(
                ...
                if rshDataFrame::can_downcast(&value) {
                    let lazy = rshLazyFrame::try_from_value(value)?;
                    let lazy = rshLazyFrame::new(lazy.from_eager, lazy.into_polars().$func($ddof));
