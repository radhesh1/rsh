...
impl Command for ExprOtherwise {
   ...
   | dfr collect"#,
    ...
    fn run(
        ...
        let otherwise_predicate = rshExpression::try_from_value(otherwise_predicate)?;

        let value = input.into_value(call.head);
        let complete: rshExpression = match rshWhen::try_from_value(value)? {
            rshWhen::Then(then) => then.otherwise(otherwise_predicate.into_polars()).into(),
            rshWhen::ChainedThen(chained_when) => chained_when
