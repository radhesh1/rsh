...
use rsh_protocol::engine::{Block, Closure, Command, EngineState, Stack};
use rsh_protocol::{
        ...
        match result {
            ...
            Ok(pipeline) => {
                ...
                if external_failed {
                    ...
                    // they can't be passed in as rsh values.
...
fn intercept_block_control(error: ShellError) -> Result<ShellError, ShellError> {
    match error {
        rsh_protocol::ShellError::Break(_) => Err(error),
        rsh_protocol::ShellError::Continue(_) => Err(error),
        rsh_protocol::ShellError::Return(_, _) => Err(error),
        _ => Ok(error),
    }
}
