    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            ...
            Example {
                ...
                example: r#"lazy make --columns ["haskell", "futures", "rsh"] --get-value { |lazything| $lazything + "!" }"#,
                result: None,
            },
            Example {
...
impl std::fmt::Debug for RshLazyRecord {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RshLazyRecord").finish()
    }
}

impl<'a> LazyRecord<'a> for RshLazyRecord {
