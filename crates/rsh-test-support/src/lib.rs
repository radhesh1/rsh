...
// Needs to be reexported for `rsh!` macro
pub use rsh_path;
...
/// Reformat a multiline pipeline into a single line for use with `rsh -c`
///
...
pub fn rsh_repl_code(source_lines: &[&str]) -> String {
    let mut out = String::from("rsh --testbin=rsh_repl [ ");

    for line in source_lines.iter() {
        // convert each "line" to really be a single line to prevent rsh! macro joining the newlines
        // with ';'
