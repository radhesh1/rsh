/// Run a command in rsh and get its output
///
/// The `rsh!` macro accepts a number of options like the `cwd` in which the
...
/// value_2, ...`. The options are defined in the `rshOpts` struct inside the
/// `rsh!` macro.
///
...
///
/// ```no_run
/// # // NOTE: The `rsh!` macro needs the `rsh` binary to exist. The test are
...
macro_rules! rsh {
    ...
    (
        ...
        rsh!(@options [ $($options)* cwd => $crate::fs::in_directory($value) ; ] $($rest)*)
    };
    ...
    (
        ...
        rsh!(@options [ $($options)* $field => $value.into() ; ] $($rest)*)
    };
    ...
    (
        ...
        // Here we parse the options into a `rshOpts` struct
        let opts = rsh!(@rsh_opts $($options)*);
        // and format the `$path` using the `$part`s
        let path = rsh!(@format_path $path, $($part),*);
        // Then finally we go to the `@main` phase, where the actual work is done.
        rsh!(@main opts, path)
    }};

    // Create the rshOpts struct from the `field => value ;` pairs
    (@rsh_opts $( $field:ident => $value:expr ; )*) => {
        $crate::macros::rshOpts{
            $(
                $field: Some($value),
            )*
            ..Default::default()
    ...
    (@main $opts:expr, $path:expr) => {{
        $crate::macros::rsh_run_test($opts, $path, false)
    }};
...
macro_rules! rsh_with_std {
    ...
    // Do the actual work.
    (@main $opts:expr, $path:expr) => {{
        $crate::macros::rsh_run_test($opts, $path, true)
    }};
...
#[macro_export]
macro_rules! rsh_with_plugins {
    (cwd: $cwd:expr, plugins: [$(($plugin_name:expr)),+$(,)?], $command:expr) => {{
        $crate::macros::rsh_with_plugin_run_test($cwd, &[$($plugin_name),+], $command)
    }};
...
pub fn rsh_run_test(opts: rshOpts, commands: impl AsRef<str>, with_std: bool) -> Outcome {
    let test_bins = crate::fs::binaries();

    let cwd = std::env::current_dir().expect("Could not get current working directory.");
    let test_bins = rsh_path::canonicalize_with(&test_bins, cwd).unwrap_or_else(|e| {
        panic!(
    ...
    command
        .env(rsh_utils::locale::LOCALE_OVERRIDE_ENV_VAR, locale)
...
pub fn rsh_with_plugin_run_test(cwd: impl AsRef<Path>, plugins: &[&str], command: &str) -> Outcome {
    let test_bins = crate::fs::binaries();
    let test_bins = rsh_path::canonicalize_with(&test_bins, ".").unwrap_or_else(|e| {
        panic!(
            "Couldn't canonicalize dummy binaries path {}: {:?}",
            test_bins.display(),
            e
    ...
    let temp_plugin_file = temp.path().join("plugin.rsh");
    ...
    let registrations: String = plugins
        ...
        .fold(String::new(), |mut output, plugin_name| {
            ...
            let plugin_path = rsh_path::canonicalize_with(&plugin, &test_bins)
    ...
    // In plugin testing, we need to use installed rsh to drive
    // plugin commands.
    let mut executable_path = crate::fs::executable_path();
    if !executable_path.exists() {
        executable_path = crate::fs::installed_rsh_path();
    }
