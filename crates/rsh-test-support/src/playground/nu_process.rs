...
pub type RshResult = Result<Outcome, RshError>;
...
impl RshProcess {
    ...
    pub fn args<T: AsRef<OsStr>>(&mut self, arguments: &[T]) -> &mut RshProcess {
        self.arguments
            .extend(arguments.iter().map(|t| t.as_ref().to_os_string()));
        self
    }

    pub fn cwd<T: AsRef<OsStr>>(&mut self, path: T) -> &mut RshProcess {
