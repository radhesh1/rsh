...
impl<'a> Playground<'a> {
    ...
    pub fn setup(topic: &str, block: impl FnOnce(Dirs, &mut Playground)) {
        ...
        let fixtures = rsh_path::canonicalize_with(fixtures.clone(), cwd).unwrap_or_else(|e| {
            panic!(
        ...
        let cwd = std::env::current_dir().expect("Could not get current working directory.");
        let test =
            rsh_path::canonicalize_with(playground_root.join(topic), cwd).unwrap_or_else(|e| {
                panic!(
        ...
        let cwd = std::env::current_dir().expect("Could not get current working directory.");
        let root = rsh_path::canonicalize_with(playground_root, cwd).unwrap_or_else(|e| {
            panic!(
