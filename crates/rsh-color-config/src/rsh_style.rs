...
pub fn parse_rsh_style(rsh_style: RshStyle) -> Style {
    let mut style = Style {
        foreground: rsh_style.fg.and_then(|fg| lookup_color_str(&fg)),
        background: rsh_style.bg.and_then(|bg| lookup_color_str(&bg)),
        ..Default::default()
    };

    if let Some(attrs) = rsh_style.attr {
...
pub fn color_record_to_rsh_style(value: &Value) -> Style {
    let mut fg = None;
    let mut bg = None;
    let mut attr = None;
    ...
    parse_rsh_style(RshStyle { fg, bg, attr })
}
...
pub fn color_from_hex(
    ...
    if trimmed.len() != 6 {
        ...
        // make a rsh_ansi_term::Color::Rgb color by converting hex to decimal
        Ok(Some(Color::Rgb(
...
fn fill_modifiers(attrs: &str, style: &mut Style) {
    // setup the attributes available in rsh_ansi_term::Style
    //
    // since we can combine styles like bold-italic, iterate through the chars
    // and set the bools for later use in the rsh_ansi_term::Style application
