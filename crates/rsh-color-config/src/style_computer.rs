...
use crate::{color_record_to_rsh_style, lookup_ansi_color_style, TextStyle};
use nu_ansi_term::{Color, Style};
use rsh_engine::{env::get_config, eval_block};
use rsh_protocol::{
...
//
// A StyleComputer is an all-in-one way to compute styles. A rsh command can
...
impl<'a> StyleComputer<'a> {
    ...
    // and a rsh value to pipe into any closures that may have been defined there.
                    ...
                    Value::Closure { val, .. } => {
                        ...
                        match eval_block(
                            ...
                            Ok(v) => {
                                ...
                                match value {
                                    Value::Record { .. } => color_record_to_rsh_style(&value),
                                    Value::String { val, .. } => lookup_ansi_color_style(&val),
                                    _ => Style::default(),
                                }
                            }
                            // This is basically a copy of rsh_cli::report_error(), but that isn't usable due to
        ...
        for (key, value) in &config.color_config {
            match value {
                ...
                Value::Record { .. } => {
                    map.insert(
                        key.to_string(),
                        ComputableStyle::Static(color_record_to_rsh_style(value)),
                    );
                }
                Value::String { val, .. } => {
...
fn test_computable_style_closure_basic() {
    use rsh_test_support::{rsh, rsh_repl_code, playground::Playground};
    Playground::setup("computable_style_closure_basic", |dirs, _| {
        ...
        let actual_repl = rsh!(cwd: dirs.test(), rsh_repl_code(&inp));
...
fn test_computable_style_closure_errors() {
    ...
    let actual_repl = rsh!(rsh_repl_code(&inp));
