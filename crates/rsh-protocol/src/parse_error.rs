...
pub enum ParseError {
    ...
    #[error("The '||' operator is not supported in Rsh")]
    #[diagnostic(
        code(rsh::parser::shell_oror),
        help("use 'try' instead of the shell '||', or 'or' instead of the boolean '||'")
    )]
    ShellOrOr(#[label("instead of '||', use 'try' or 'or'")] Span),

    #[error("The '2>' shell operation is 'err>' in Rsh.")]
    #[diagnostic(code(rsh::parser::shell_err))]
    ShellErrRedirect(#[label("use 'err>' instead of '2>' in Rsh")] Span),

    #[error("The '2>&1' shell operation is 'out+err>' in Rsh.")]
    #[diagnostic(
        code(rsh::parser::shell_outerr),
        help("Rsh redirection will write all of stdout before stderr.")
    )]
    ShellOutErrRedirect(#[label("use 'out+err>' instead of '2>&1' in Rsh")] Span),

    #[error("{0} is not supported on values of type {3}")]
    #[diagnostic(code(rsh::parser::unsupported_operation))]
    ...
    #[diagnostic(code(rsh::parser::expected_keyword))]
    ...
    #[diagnostic(
        code(rsh::parser::unexpected_keyword),
    ...
    #[diagnostic(
        code(rsh::parser::cant_alias_keyword),
    ...
    #[diagnostic(
        code(rsh::parser::module_not_found),
    ...
    #[error("Missing mod.rsh file.")]
    #[diagnostic(
        code(rsh::parser::module_missing_mod_nu_file),
        help("Directory {0} is missing a mod.rsh file.\n\nWhen importing a directory as a Rsh module, it needs to contain a mod.rsh file (can be empty). Alternatively, you can use .rsh files in the directory as modules individually.")
    )]
    ModuleMissingModRshFile(
        String,
        #[label = "module directory is missing a mod.rsh file"] Span,
    ),

    #[error("Cyclical module import.")]
    #[diagnostic(code(rsh::parser::cyclical_module_import), help("{0}"))]
    CyclicalModuleImport(String, #[label = "detected cyclical module import"] Span),

    #[error("Can't export {0} named same as the module.")]
    #[diagnostic(
        code(rsh::parser::named_as_module),
    ...
    #[diagnostic(
        code(rsh::parser::module_double_main),
    ...
    #[diagnostic(
        code(rsh::parser::invalid_module_file_name),
        help("File {0} resolves to module name {1} which is the same as the parent module. Either rename the file or, save it as 'mod.rsh' to define the parent module.")
    )]
    ...
    #[diagnostic(
        code(rsh::parser::export_main_alias_not_allowed),
    ...
    #[diagnostic(code(rsh::parser::active_overlay_not_found))]
    ActiveOverlayNotFound(#[label = "not an active overlay"] Span),

    #[error("Overlay prefix mismatch.")]
    #[diagnostic(
        code(rsh::parser::overlay_prefix_mismatch),
    ...
    #[diagnostic(
        code(rsh::parser::module_or_overlay_not_found),
    ...
    #[diagnostic(
        code(rsh::parser::cant_remove_last_overlay),
    ...
    #[diagnostic(
        code(rsh::parser::cant_hide_default_overlay),
    ...
    #[diagnostic(code(rsh::parser::cant_add_overlay_help), help("{0}"))]
    ...
    #[diagnostic(code(rsh::parser::duplicate_command_def))]
    DuplicateCommandDef(#[label = "defined more than once"] Span),

    #[error("Unknown command.")]
    #[diagnostic(
        code(rsh::parser::unknown_command),
    ...
    #[diagnostic(code(rsh::parser::unknown_flag), help("{3}"))]
    UnknownFlag(String, String, #[label = "unknown flag"] Span, String),

    #[error("Unknown type.")]
    #[diagnostic(code(rsh::parser::unknown_type))]
    UnknownType(#[label = "unknown type"] Span),

    #[error("Missing flag argument.")]
    #[diagnostic(code(rsh::parser::missing_flag_param))]
    ...
    #[diagnostic(code(rsh::parser::only_last_flag_in_batch_can_take_arg))]
    OnlyLastFlagInBatchCanTakeArg(#[label = "only the last flag can take args"] Span),

    #[error("Missing required positional argument.")]
    #[diagnostic(
        code(rsh::parser::missing_positional),
    ...
    #[diagnostic(code(rsh::parser::keyword_missing_arg))]
    ...
    #[error("Missing type.")]
    #[diagnostic(code(rsh::parser::missing_type))]
    ...
    #[diagnostic(code(rsh::parser::non_constant_default_value))]
    NonConstantDefaultValue(#[label = "expected a constant value"] Span),

    #[error("Extra columns.")]
    #[diagnostic(code(rsh::parser::extra_columns))]
    ExtraColumns(
        usize,
    ...
    #[diagnostic(code(rsh::parser::missing_columns))]
    MissingColumns(
        usize,
        #[label("expected {0} column{}", if *.0 == 1 { "" } else { "s" })] Span,
    ),
    ...
    #[error("{0}")]
    #[diagnostic(code(rsh::parser::assignment_mismatch))]
    AssignmentMismatch(String, String, #[label("{1}")] Span),

    #[error("Missing import pattern.")]
    #[diagnostic(code(rsh::parser::missing_import_pattern))]
    ...
    #[diagnostic(code(rsh::parser::export_not_found))]
    ExportNotFound(#[label = "could not find imports"] Span),

    #[error("File not found")]
    #[diagnostic(
        code(rsh::parser::sourced_file_not_found),
    ...
    #[error("File not found")]
    #[diagnostic(
        code(rsh::parser::registered_file_not_found),
    ...
    #[error("File not found")]
    #[diagnostic(code(rsh::parser::file_not_found))]
    ...
    #[diagnostic(code(rsh::parser::unexpected_spread_operator))]
...
impl ParseError {
    pub fn span(&self) -> Span {
        match self {
            ...
            ParseError::ModuleMissingModRshFile(_, s) => *s,
