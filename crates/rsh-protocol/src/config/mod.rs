...
pub use self::reedline::{
    create_menus, EditBindings, HistoryFileFormat, RshCursorShape, ParsedKeybinding, ParsedMenu,
};
...
pub struct Config {
    ...
    pub cursor_shape_vi_insert: RshCursorShape,
    pub cursor_shape_vi_normal: RshCursorShape,
    pub cursor_shape_emacs: RshCursorShape,
...
impl Default for Config {
    fn default() -> Config {
        Config {
            ...
            cursor_shape_emacs: RshCursorShape::default(),
            cursor_shape_vi_insert: RshCursorShape::default(),
            cursor_shape_vi_normal: RshCursorShape::default(),
...
impl Value {
    pub fn into_config(&mut self, config: &Config) -> (Config, Option<ShellError>) {
        ...
        // Vec for storing errors. Current rsh behaviour (Dec 2022) is that having some typo
        // like `"always_trash": tru` in your config.rsh's `$env.config` record shouldn't abort all
        ...
        // with a reconstructed rsh value for the current (unaltered) configuration for that setting.
        // For instance:
