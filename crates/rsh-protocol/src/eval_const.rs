...
use rsh_system::os_info::{get_kernel_version, get_os_arch, get_os_family, get_os_name};
use std::path::{Path, PathBuf};

pub fn create_rsh_constant(engine_state: &EngineState, span: Span) -> Result<Value, ShellError> {
    fn canonicalize_path(engine_state: &EngineState, path: &Path) -> PathBuf {
        let cwd = engine_state.current_work_dir();

        if path.exists() {
            match rsh_path::canonicalize_with(path, cwd) {
    ...
    record.push(
        "default-config-dir",
        if let Some(mut path) = rsh_path::config_dir() {
            path.push("rsh");
    ...
    record.push(
        ...
        } else if let Some(mut path) = rsh_path::config_dir() {
            path.push("rsh");
            path.push("config.rsh");
            Value::string(path.to_string_lossy(), span)
        } else {
            Value::error(
                ShellError::IOError("Could not get config directory".into()),
    ...
    #[cfg(feature = "plugin")]
    {
        record.push(
            ...
            if let Some(path) = &engine_state.plugin_signatures {
                ...
                plugin_path.push("rsh");
                plugin_path.push("plugin.rsh");
    ...
    record.push(
        "home-path",
        if let Some(path) = rsh_path::home_dir() {
