...
/// RwLock, but you can use Arc by itself. We did this a few places in the original rsh. This *can* work
...
/// more closely to what we need with rsh. And, of course, it's still thread-safe, so we get the same
/// benefits as above.
///
#[derive(Clone)]
pub struct EngineState {
    ...
    // If rsh was started, e.g., with `rsh spam.rsh`, the file's parent is stored here
...
pub const RSH_VARIABLE_ID: usize = 0;
...
impl EngineState {
    ...
    /// Mark a starting point if it is a script (e.g., rsh spam.rsh)
...
mod engine_state_tests {
    ...
    fn add_file_gives_id() {
        ...
        let id = engine_state.add_file("test.rsh".into(), &[]);
    ...
    fn add_file_gives_id_including_parent() {
        ...
        let parent_id = engine_state.add_file("test.rsh".into(), vec![]);

        let mut working_set = StateWorkingSet::new(&engine_state);
        let working_set_id = working_set.add_file("child.rsh".into(), &[]);
    ...
    fn merge_states() -> Result<(), ShellError> {
        let mut engine_state = EngineState::new();
        engine_state.add_file("test.rsh".into(), vec![]);

        let delta = {
            let mut working_set = StateWorkingSet::new(&engine_state);
            let _ = working_set.add_file("child.rsh".into(), &[]);
            working_set.render()
        };
        ...
        assert_eq!(&engine_state.files[0].0, "test.rsh");
        assert_eq!(&engine_state.files[1].0, "child.rsh");
