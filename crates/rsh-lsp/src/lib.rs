...
use rsh_parser::{flatten_block, parse, FlatShape};
use rsh_protocol::{
...
impl LanguageServer {
    ...
    fn complete(
        ...
        let stack = Stack::new();
        let mut completer = RshCompleter::new(Arc::new(engine_state.clone()), stack);
...
mod tests {
    ...
    use rsh_test_support::fs::{fixtures, root};
    ...
    fn goto_definition_for_none_existing_file() {
        ...
        none_existent_path.push("none-existent.rsh");
    ...
    fn complete_command_with_space() {
        ...
        assert_json_eq!(
            ...
            serde_json::json!([
               {
                  "label": "config rsh",
                  "detail": "Edit rsh configurations.",
                  "textEdit": {
                     "range": {
