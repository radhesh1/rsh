...
use rsh_protocol::{record, ListStream, PipelineData, PipelineMetadata, RawStream, Value};
...
pub fn map_into_value(hm: HashMap<String, Value>) -> Value {
    Value::record(hm.into_iter().collect(), rshSpan::unknown())
}

fn unknown_error_value() -> Value {
    Value::string(String::from("❎"), rshSpan::unknown())
}
