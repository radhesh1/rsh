...
pub fn lscolorize(header: &[String], data: &mut [Vec<RshText>], lscolors: &LsColors) {
...
fn get_path_style(path: &str, ls_colors: &LsColors) -> Option<Style> {
    ...
    style.map(lsstyle_to_rsh_style)
}

fn lsstyle_to_rsh_style(s: &lscolors::Style) -> Style {
    let mut out = Style::default();
    if let Some(clr) = &s.background {
        out.background = lscolor_to_rsh_color(clr);
    }

    if let Some(clr) = &s.foreground {
        out.foreground = lscolor_to_rsh_color(clr);
    }
...
fn lscolor_to_rsh_color(clr: &lscolors::Color) -> Option<Color> {
    use lscolors::Color::*;
