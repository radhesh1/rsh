...
impl ViewCommand for ExpandCmd {
    ...
    fn help(&self) -> Option<HelpManual> {
        ...
        Some(HelpManual {
            name: "expand",
            description:
                "View the currently selected cell's data using the `table` rsh command",
            arguments: vec![],
            examples: vec![],
            config_options: vec![],
            input: shortcodes,
...
fn convert_value_to_string(value: Value, engine_state: &EngineState, stack: &mut Stack) -> String {
    ...
    if !has_no_head && has_single_value {
        ...
        rsh_common::try_build_table(ctrlc, config, &style_computer, value)
    }
}
