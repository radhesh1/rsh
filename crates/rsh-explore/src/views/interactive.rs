...
use rsh_color_config::get_color_map;
use rsh_protocol::{
...
use crate::{
    rsh_common::{collect_pipeline, run_command_with_value},
...
use super::{
    ...
    util::{lookup_tui_color, rsh_style_to_tui},
    ...
    fn setup(&mut self, config: ViewConfig<'_>) {
        ...
        if let Some(hm) = config.config.get("try").and_then(create_map) {
            ...
            if let Some(color) = colors.get("highlighted_color").copied() {
                self.highlighted_color = rsh_style_to_tui(color);
            }
