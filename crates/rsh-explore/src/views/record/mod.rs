...
use rsh_color_config::{get_color_map, StyleComputer};
use rsh_protocol::{
...
use crate::{
    rsh_common::{collect_input, lscolorize, rshConfig, rshSpan, rshStyle, RshText},
    pager::{
...
use super::{
    cursor::XYCursor,
    util::{make_styled_string, rsh_style_to_tui},
...
impl<'a> RecordView<'a> {
    ...
    pub fn set_style_split_line(&mut self, style: rshStyle) {
        self.theme.table.splitline_style = style
    }

    pub fn set_style_selected_cell(&mut self, style: rshStyle) {
        self.theme.cursor.selected_cell = Some(style)
    }

    pub fn set_style_selected_row(&mut self, style: rshStyle) {
        self.theme.cursor.selected_row = Some(style)
    }

    pub fn set_style_selected_column(&mut self, style: rshStyle) {
    ...
    fn create_tablew(&'a self, cfg: ViewConfig<'a>) -> TableW<'a> {
        let layer = self.get_layer_last();
        let mut data = convert_records_to_string(&layer.records, cfg.rsh_config, cfg.style_computer);
...
impl View for RecordView<'_> {
    ...
    fn collect_data(&self) -> Vec<RshText> {
...
fn highlight_cell(f: &mut Frame, area: Rect, info: ElementInfo, theme: &CursorStyle) {
    ...
    let highlight_block = Block::default().style(rsh_style_to_tui(cell_style));
...
fn build_table_as_list(v: &RecordView) -> Value {
    ...
    Value::list(vals, rshSpan::unknown())
}
...
fn transpose_table(layer: &mut RecordLayer<'_>) {
    ...
    if layer.was_transposed {
        ...
        for (column, column_name) in layer.columns.iter().enumerate() {
            let value = Value::string(column_name, rshSpan::unknown());
...
#[derive(Debug, Default, Clone)]
struct CursorStyle {
    selected_cell: Option<rshStyle>,
    selected_column: Option<rshStyle>,
    selected_row: Option<rshStyle>,
}
