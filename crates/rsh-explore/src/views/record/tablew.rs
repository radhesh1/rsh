...
use crate::{
    rsh_common::{truncate_str, rshStyle, RshText},
    views::util::{rsh_style_to_tui, text_style_to_tui_style},
};
...
pub struct TableStyle {
    pub splitline_style: rshStyle,
    pub shift_line_style: rshStyle,
    pub show_index: bool,
...
impl<'a> TableW<'a> {
    ...
    pub fn new(
        ...
        data: impl Into<Cow<'a, [Vec<RshText>]>>,
        style_computer: &'a StyleComputer<'a>,
        index_row: usize,
        index_column: usize,
        style: TableStyle,
...
impl Widget for IndexColumn<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        for row in 0..area.height {
            ...
            let style = rsh_style_to_tui(self.style_computer.compute(
                "row_index",
                &Value::string(text.as_str(), rsh_protocol::Span::unknown()),
            ));
...
fn render_header_borders(buf: &mut Buffer, area: Rect, span: u16, style: rshStyle) -> (u16, u16) {
    let borders = Borders::TOP | Borders::BOTTOM;
    let block = Block::default()
        .borders(borders)
        .border_style(rsh_style_to_tui(style));
    let height = span + 2;
...
fn render_vertical_split(buf: &mut Buffer, x: u16, y: u16, height: u16, style: rshStyle) {
    let style = TextStyle {
...
fn create_column(data: &[Vec<RshText>], col: usize) -> Vec<RshText> {
    let mut column = vec![RshText::default(); data.len()];
...
fn truncate_list(list: &mut [RshText], width: usize) {
    for (text, _) in list {
        truncate_str(text, width);
    }
}

fn render_shift_column(buf: &mut Buffer, x: u16, y: u16, height: u16, style: rshStyle) -> u16 {
    let style = TextStyle {
        alignment: Alignment::Left,
        color_style: Some(style),
    };
...
fn render_top_connector(buf: &mut Buffer, x: u16, y: u16, style: rshStyle) {
    let style = rsh_style_to_tui(style);
    let span = Span::styled("┬", style);
    buf.set_span(x, y, &span, 1);
}

fn render_bottom_connector(buf: &mut Buffer, x: u16, y: u16, style: rshStyle) {
    let style = rsh_style_to_tui(style);
    let span = Span::styled("┴", style);
    buf.set_span(x, y, &span, 1);
}

fn calculate_column_width(column: &[RshText]) -> usize {
    column
        .iter()
        .map(|(text, _)| text)
...
fn head_row_text(head: &str, style_computer: &StyleComputer) -> RshText {
    (
        String::from(head),
        TextStyle::with_style(
            Alignment::Center,
            style_computer.compute("header", &Value::string(head, rsh_protocol::Span::unknown())),
        ),
    )
}
