...
use commands::{ExpandCmd, HelpCmd, HelpManual, RshCmd, QuitCmd, TableCmd, TryCmd};
use rsh_common::{collect_pipeline, has_simple_value, CtrlC};
use rsh_protocol::{
...
mod util {
    pub use super::rsh_common::{create_lscolors, create_map, map_into_value};
}
...
fn run_pager(
    ...
    if let Some(value) = has_simple_value(&data) {
        let text = value.into_abbreviated_string(config.rsh_config);
...
fn create_commands(registry: &mut CommandRegistry) {
    registry.register_command_view(RshCmd::new(), false);
