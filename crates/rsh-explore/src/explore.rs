...
impl Command for Explore {
    ...
    fn run(
        ...
        let rsh_config = engine_state.get_config();
        let style_computer = StyleComputer::from_config(engine_state, stack);

        let mut config = rsh_config.explore.clone();
        include_rsh_config(&mut config, &style_computer);
        ...
        let mut config = PagerConfig::new(rsh_config, &style_computer, &lscolors, config);
        config.style = style;
...
fn insert_style(map: &mut HashMap<String, Value>, key: &str, value: Style) {
    ...
    let value = rsh_color_config::RshStyle::from(value);
    if let Ok(val) = rsh_json::to_string_raw(&value) {
...
fn include_rsh_config(config: &mut HashMap<String, Value>, style_computer: &StyleComputer) {
