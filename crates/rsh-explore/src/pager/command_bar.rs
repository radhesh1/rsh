...
impl<'a> CommandBar<'a> {
    pub fn new(text: &'a str, information: &'a str, text_s: rshStyle, back_s: rshStyle) -> Self {
        let text_s = rsh_style_to_tui(text_s).add_modifier(Modifier::BOLD);
        let back_s = rsh_style_to_tui(back_s);
