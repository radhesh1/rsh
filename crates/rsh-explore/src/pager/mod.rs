...
use rsh_color_config::{lookup_ansi_color_style, StyleComputer};
use rsh_protocol::{
...
use crate::{
    rsh_common::{CtrlC, rshColor, rshConfig, rshSpan, rshStyle},
    registry::{Command, CommandRegistry},
    util::map_into_value,
    views::{util::rsh_style_to_tui, ViewConfig},
};
...
pub struct StyleConfig {
    pub status_info: rshStyle,
    pub status_success: rshStyle,
    pub status_warn: rshStyle,
    pub status_error: rshStyle,
    pub status_bar_background: rshStyle,
    pub status_bar_text: rshStyle,
    pub cmd_bar_text: rshStyle,
    pub cmd_bar_background: rshStyle,
    pub highlight: rshStyle,
}
...
fn create_view_config<'a>(pager: &'a Pager<'_>) -> ViewConfig<'a> {
    ...
    ViewConfig::new(cfg.rsh_config, cfg.style_computer, &cfg.config, cfg.lscolors)
}
...
fn update_view_setup(view: &mut Box<dyn View>, cfg: &PagerConfig<'_>) {
    let cfg = ViewConfig::new(cfg.rsh_config, cfg.style_computer, &cfg.config, cfg.lscolors);
    view.setup(cfg);
}
...
fn report_msg_style(report: &Report, theme: &StyleConfig, style: rshStyle) -> rshStyle {
...
fn render_cmd_bar_search(f: &mut Frame, area: Rect, pager: &Pager<'_>, theme: &StyleConfig) {
    if pager.search_buf.search_results.is_empty() && !pager.search_buf.is_search_input {
        ...
        let style = rshStyle {
            background: Some(rshColor::Red),
            foreground: Some(rshColor::White),
            ..Default::default()
        };
...
fn highlight_search_results(f: &mut Frame, pager: &Pager, layout: &Layout, style: rshStyle) {
    if pager.search_buf.search_results.is_empty() {
        return;
    }

    let highlight_block = Block::default().style(rsh_style_to_tui(style));
...
fn set_config(hm: &mut HashMap<String, Value>, path: &[&str], value: Value) -> bool {
    ...
    if !hm.contains_key(key) {
        hm.insert(
            key.to_string(),
            Value::record(Record::new(), rshSpan::unknown()),
        );
    }
...
fn report_level_style(level: Severity, theme: &StyleConfig) -> rshStyle {
    match level {
