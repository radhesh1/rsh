...
use rsh_plugin::{serve_plugin, MsgPackSerializer, Plugin};
use rsh_plugin::{EvaluatedCall, LabeledError};
use rsh_protocol::{Category, PluginSignature, ShellError, Value};
use second_custom_value::SecondCustomValue;

struct CustomValuePlugin;

impl Plugin for CustomValuePlugin {
    fn signature(&self) -> Vec<rsh_protocol::PluginSignature> {
        vec![
