...
fn list_annotations_unknown_inner_type() -> TestResult {
    let input = "def run [list: list<str>] {$list | length}; run ['rsh' 'nunu' 'nana']";
    let expected = "unknown type";
    fail_test(input, expected)
}
...
fn list_annotations_nested_unknown_inner() -> TestResult {
    let input = "def run [list: list<list<str>>] {$list | length}; run [ [rsh] [nunu] [nana]]";
    let expected = "unknown type";
    fail_test(input, expected)
}
...
fn record_annotations_two_types() -> TestResult {
    let input = "def run [rec: record<name: string age: int>] { $rec }; run {name: rsh age: 3} | describe";
...
fn record_annotations_key_with_no_type() -> TestResult {
    let input = "def run [rec: record<name>] { $rec }; run {name: rsh} | describe";
...
fn record_annotations_two_types_one_with_no_type() -> TestResult {
    let input =
        "def run [rec: record<name: string, age>] { $rec }; run {name: rsh age: 3} | describe";
    let expected = "record<name: string, age: int>";
    run_test(input, expected)
}
...
fn record_annotations_two_types_both_with_no_types() -> TestResult {
    let input = "def run [rec: record<name age>] { $rec }; run {name: rsh age: 3} | describe";
    let expected = "record<name: string, age: int>";
    run_test(input, expected)
}
...
fn record_annotations_type_mismatch_key() -> TestResult {
    let input = "def run [rec: record<name: string>] { $rec }; run {nme: rsh}";
...
fn table_annotations_key_with_no_type() -> TestResult {
    let input = "def run [t: table<name>] { $t }; run [[name]; [rsh]] | describe";
...
fn table_annotations_type_mismatch_column() -> TestResult {
    let input = "def run [t: table<name: string>] { $t }; run [[nme]; [rsh]]";
