...
fn quotes_with_equals() -> TestResult {
    run_test(
        r#"let query_prefix = "https://api.github.com/search/issues?q=repo:rsh/"; $query_prefix"#,
        "https://api.github.com/search/issues?q=repo:rsh/",
    )
}
...
fn equals_separates_long_flag() -> TestResult {
    run_test(
        r#"'rsh' | fill --alignment right --width=10 --character='-'"#,
        "---rsh",
    )
}
...
fn register_with_string_literal() -> TestResult {
    fail_test(r#"register 'rsh-plugin-math'"#, "File not found")
}

#[test]
fn register_with_string_constant() -> TestResult {
    let input = "\
const file = 'rsh-plugin-math'
register $file
";
