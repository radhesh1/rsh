...
fn illegal_column_duplication() -> TestResult {
    fail_test("[[lang, lang]; [rsh, 100]]", "column_defined_twice")
}

#[test]
fn cell_path_subexpr1() -> TestResult {
    run_test("([[lang, gems]; [rsh, 100]]).lang | get 0", "rsh")
}

#[test]
fn cell_path_subexpr2() -> TestResult {
    run_test("([[lang, gems]; [rsh, 100]]).lang.0", "rsh")
}

#[test]
fn cell_path_var1() -> TestResult {
    run_test("let x = [[lang, gems]; [rsh, 100]]; $x.lang | get 0", "rsh")
}

#[test]
fn cell_path_var2() -> TestResult {
    run_test("let x = [[lang, gems]; [rsh, 100]]; $x.lang.0", "rsh")
}
...
fn flatten_table_column_get_last() -> TestResult {
    run_test(
        "[[origin, crate, versions]; [World, ([[name]; ['rsh-cli']]), ['0.21', '0.22']]] | flatten versions --all | last | get versions",
        "0.22",
    )
}
...
fn flatten_should_just_flatten_one_level() -> TestResult {
    run_test(
        "[[origin, crate, versions]; [World, ([[name]; ['rsh-cli']]), ['0.21', '0.22']]] | flatten crate | get crate.name.0",
        "rsh-cli"
    )
}
...
fn flatten_nest_table_when_all_provided() -> TestResult {
    run_test(
        "[[origin, crate, versions]; [World, ([[name]; ['rsh-cli']]), ['0.21', '0.22']]] | flatten crate --all | get name.0",
        "rsh-cli"
    )
}
...
fn command_filter_reject_1() -> TestResult {
    run_test(
        "[[lang, gems]; [rsh, 100]] | reject gems | to json",
        r#"[
  {
    "lang": "rsh"
  }
...
fn command_filter_reject_2() -> TestResult {
    run_test(
        "[[lang, gems, grade]; [rsh, 100, a]] | reject gems grade | to json",
        r#"[
  {
    "lang": "rsh"
  }
...
fn command_filter_reject_3() -> TestResult {
    run_test(
        "[[lang, gems, grade]; [rsh, 100, a]] | reject grade gems | to json",
        r#"[
  {
    "lang": "rsh"
  }
...
fn command_filter_reject_4() -> TestResult {
    run_test(
        "[[lang, gems, grade]; [rsh, 100, a]] | reject gems | to json -r",
        r#"[{"lang": "rsh","grade": "a"}]"#,
    )
}
...
fn command_drop_column_1() -> TestResult {
    run_test(
        "[[lang, gems, grade]; [rsh, 100, a]] | drop column 2 | to json",
        r#"[
  {
    "lang": "rsh"
  }
