...
use rsh_protocol::{
    ...
    report_error, DeclId, ShellError, Span, Value, VarId, RSH_VARIABLE_ID,
};
...
pub fn check(engine_state: &mut EngineState, file_path: &str, max_errors: &Value) {
    ...
    let rsh_const = match create_rsh_constant(engine_state, Span::unknown()) {
        Ok(rsh_const) => rsh_const,
        Err(err) => {
...
pub fn complete(engine_reference: Arc<EngineState>, file_path: &str, location: &Value) {
    let stack = Stack::new();
    let mut completer = RshCompleter::new(engine_reference, stack);
