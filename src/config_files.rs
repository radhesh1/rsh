...
use rsh_cli::{eval_config_contents, eval_source};
use rsh_path::canonicalize_with;
use rsh_protocol::engine::{EngineState, Stack, StateWorkingSet};
use rsh_protocol::report_error;
use rsh_protocol::{ParseError, PipelineData, Spanned};
use rsh_utils::{get_default_config, get_default_env};
use std::fs::File;
use std::io::Write;
use std::path::Path;

pub(crate) const RSH_FOLDER: &str = "rsh";
const CONFIG_FILE: &str = "config.rsh";
const ENV_FILE: &str = "env.rsh";
const LOGINSHELL_FILE: &str = "login.rsh";
...
pub(crate) fn read_config_file(
    ...
    } else if let Some(mut config_path) = rsh_path::config_dir() {
        config_path.push(RSH_FOLDER);
...
pub(crate) fn read_loginshell_file(engine_state: &mut EngineState, stack: &mut Stack) {
    ...
    if let Some(mut config_path) = rsh_path::config_dir() {
        config_path.push(RSH_FOLDER);
...
pub(crate) fn read_default_env_file(engine_state: &mut EngineState, stack: &mut Stack) {
    ...
    match rsh_engine::env::current_dir(engine_state, stack) {
        Ok(cwd) => {
...
fn eval_default_config(
    ...
    // Just use the contents of "default_config.rsh" or "default_env.rsh"
    eval_source(
        engine_state,
        stack,
        config_file.as_bytes(),
...
pub(crate) fn setup_config(
    ...
    #[cfg(feature = "plugin")]
    read_plugin_file(engine_state, stack, plugin_file, RSH_FOLDER);
...
pub(crate) fn set_config_path(
    ...
    let config_path = match config_file {
        ...
        None => rsh_path::config_dir().map(|mut p| {
            p.push(RSH_FOLDER);
