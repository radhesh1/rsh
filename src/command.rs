use rsh_engine::{get_full_help, CallExt};
use rsh_parser::parse;
use rsh_parser::{escape_for_script_arg, escape_quote_string};
use rsh_protocol::report_error;
use rsh_protocol::{
...
use rsh_utils::stdout_write_all_and_flush;

pub(crate) fn gather_commandline_args() -> (Vec<String>, String, Vec<String>) {
    // Would be nice if we had a way to parse this. The first flags we see will be going to rsh
    // then it'll be the script name
    // then the args to the script
    let mut args_to_rsh = Vec::from(["rsh".into()]);
    ...
    while let Some(arg) = args.next() {
        ...
        args_to_rsh.push(arg);

        if let Some(flag_value) = flag_value {
            args_to_rsh.push(flag_value);
        }
    }
    ...
    (args_to_rsh, script_name, args_to_script)
}

pub(crate) fn parse_commandline_args(
    commandline_args: &str,
    engine_state: &mut EngineState,
) -> Result<RshCliArgs, ShellError> {
    let (block, delta) = {
        let mut working_set = StateWorkingSet::new(engine_state);
        working_set.add_decl(Box::new(Rsh));
...
impl Command for Rsh {
    fn name(&self) -> &str {
        "rsh"
    }

    fn signature(&self) -> Signature {
        let mut signature = Signature::build("Rsh")
            .usage("The rsh language and shell.")
            .named(
                "commands",
                SyntaxShape::String,
            ...
            .named(
                "include-path",
                SyntaxShape::String,
                "set the rsh_LIB_DIRS for the given script (delimited by char record_sep ('\x1e'))",
                Some('I'),
            )
            ...
            .switch(
                "lsp",
                "start Rsh's language server protocol",
                None,
            )
            .named(
                "ide-goto-def",
    ...
    fn run(
        ...
        Ok(Value::string(
            get_full_help(&Rsh.signature(), &Rsh.examples(), engine_state, stack, true),
            call.head,
        )
            .into_pipeline_data())
    }

    fn examples(&self) -> Vec<rsh_protocol::Example> {
        vec![
            Example {
                description: "Run a script",
                example: "rsh myfile.rsh",
                result: None,
            },
            Example {
                description: "Run rsh interactively (as a shell or REPL)",
                example: "Rsh",
                result: None,
            },
        ]
