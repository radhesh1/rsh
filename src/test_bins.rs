use rsh_cmd_base::hook::{eval_env_change_hook, eval_hook};
use rsh_engine::eval_block;
use rsh_parser::parse;
use rsh_protocol::engine::{EngineState, Stack, StateWorkingSet};
use rsh_protocol::{CliError, PipelineData, Value};
use rsh_std::load_standard_library;
use std::io::{self, BufRead, Read, Write};

/// Echo's value of env keys from args
/// Example: rsh --testbin env_echo FOO BAR
...
/// Example: rsh --testbin echo a b c
/// a b c
pub fn cococo() {
...
/// rsh --testbin repeater a 5
/// aaaaa
/// rsh --testbin repeater test 5
...
/// Generate a minimal engine state with just `rsh-cmd-lang`, `rsh-command`, and `rsh-cli` commands.
fn get_engine_state() -> EngineState {
    ...
    rsh_cli::add_cli_context(engine_state)
}
...
pub fn rsh_repl() {
    ...
    for (i, line) in source_lines.iter().enumerate() {
        let cwd = rsh_engine::env::current_dir(&engine_state, &stack)
