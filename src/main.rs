...
use log::Level;
use miette::Result;
use rsh_cli::gather_parent_env_vars;
use rsh_cmd_base::util::get_init_cwd;
use rsh_lsp::LanguageServer;
use rsh_protocol::{
    engine::EngineState, eval_const::create_rsh_constant, report_error_new, util::BufferedReader,
    PipelineData, RawStream, Span, Value, RSH_VARIABLE_ID,
};
use rsh_std::load_standard_library;
...
fn get_engine_state() -> EngineState {
    let engine_state = rsh_cmd_lang::create_default_context();
    let engine_state = rsh_command::add_shell_command_context(engine_state);
    #[cfg(feature = "extra")]
        let engine_state = rsh_cmd_extra::add_extra_command_context(engine_state);
    #[cfg(feature = "dataframe")]
        let engine_state = rsh_cmd_dataframe::add_dataframe_context(engine_state);
    let engine_state = rsh_cli::add_cli_context(engine_state);
    rsh_explore::add_explore_context(engine_state)
}
...
fn main() -> Result<()> {
    ...
    // Custom additions
    let delta = {
        let mut working_set = rsh_protocol::engine::StateWorkingSet::new(&engine_state);
        working_set.add_decl(Box::new(rsh_cli::RshHighlight));
        working_set.add_decl(Box::new(rsh_cli::Print));
        working_set.render()
    };
    ...
    let (args_to_rsh, script_name, args_to_script) = gather_commandline_args();
    let parsed_nu_cli_args = parse_commandline_args(&args_to_rsh.join(" "), &mut engine_state)
