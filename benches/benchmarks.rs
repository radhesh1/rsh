...
use rsh_cli::eval_source;
use rsh_parser::parse;
use rsh_plugin::{EncodingType, PluginResponse};
use rsh_protocol::{engine::EngineState, PipelineData, Span, Value};
...
fn parser_benchmarks(c: &mut Criterion) {
    ...
    // parsing config.rsh breaks without PWD set
    ...
    c.bench_function("parse_default_env_file", |b| {
        b.iter_batched(
            || rsh_protocol::engine::StateWorkingSet::new(&engine_state),
    ...
    c.bench_function("eval default_env.rsh", |b| {
        b.iter(|| {
            let mut engine_state = load_bench_commands();
    ...
    c.bench_function("eval default_config.rsh", |b| {
        b.iter(|| {
            let mut engine_state = load_bench_commands();
            // parsing config.rsh breaks without PWD set
            engine_state.add_env_var(
